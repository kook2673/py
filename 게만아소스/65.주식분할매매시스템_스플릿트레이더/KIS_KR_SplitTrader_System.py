    ...
    for AutoSplitData in AutoOrderList:
        ...
        if AutoSplitData['OrderType'] == "Sell":
            ...
            if AutoSplitData['TimeCnt'] >= AutoSplitData['TimeTerm']:
                ...
                msg = DIST + " " + AutoSplitData['stock_code'] + " " + KisKR.GetStockName(AutoSplitData['stock_code']) + " " +  str(AutoSplitData['OrderVolume']) + "개 매도 중.." + str(AutoSplitData['RemainingVolume']) + "개 남음"
                print(msg)
                ...
                if AutoSplitData['RemainingVolume'] < 1:
                    ...
                    msg = DIST + " " + AutoSplitData['stock_code'] + " " + KisKR.GetStockName(AutoSplitData['stock_code']) + " " +  str(AutoSplitData['OrderVolume']) + "개의 모든 매도 주문이 완료가 되어 분할 매도 종료합니다!"
                    print(msg)
                    line_alert.SendMessage(msg)
                    ...
                    if SellVolume >= 1:
                        ...
                        #data = KisKR.MakeSellLimitOrder(AutoSplitData['stock_code'], SellVolume, nowPrice*0.99)
                        print(data)
                        time.sleep(0.2)
        ...
        if AutoSplitData['OrderType'] == "Buy":
            ...
            if AutoSplitData['TimeCnt'] >= AutoSplitData['TimeTerm']:
                ...
                if AutoSplitData['OrderCnt'] >= AutoSplitData['SplitCount']:
                    ...
                    msg = DIST + " " + AutoSplitData['stock_code'] + " " + KisKR.GetStockName(AutoSplitData['stock_code']) + " " +  str(AutoSplitData['OrderVolume']) + "개 매수 중.." + str(AutoSplitData['RemainingVolume']) + "개 남음"
                    print(msg)
                    line_alert.SendMessage(msg)
                ...
                if AutoSplitData['RemainingVolume'] < 1:
                    ...
                    msg = DIST + " " + AutoSplitData['stock_code'] + " " + KisKR.GetStockName(AutoSplitData['stock_code']) + " " +  str(AutoSplitData['OrderVolume']) + "개의 모든 매수 주문이 완료가 되어 분할 매수 종료합니다!"
                    print(msg)
                    line_alert.SendMessage(msg)
                    ...
                    if BuyVolume >= 1:
                        ...
                        #첫번째 매수!
                        data = KisKR.MakeBuyMarketOrder(AutoSplitData['stock_code'],BuyVolume)
                        #data = KisKR.MakeBuyLimitOrder(AutoSplitData['stock_code'],BuyVolume,nowPrice*1.01)
                        print(data)
                        time.sleep(0.2)
