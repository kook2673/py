...
'''
StockCodeList = [] #아래 같은 예시로 종목을 추가하셔요
...
#포트폴리오 이름
PortfolioName = "게만아 미국 지수 추종 섹터 ETF 전략 업그레이드!"
...
for stock_code in StockCodeList:
    ...
    asset['stock_target_rate'] = 16.0    #포트폴리오 목표 비중 총 5개니깐 16%씩 할당된다.
...
asset['stock_target_rate'] = 20.0   #SPY에 20%
asset['stock_rebalance_amt'] = 0     #리밸런싱 해야하는 수량
...
for stock_info in MyPortfolioList:
    ...
    stock_info['avg_momentum_score'] = 0

    #이건 평균 모멘텀을 구하기에 상장된지 얼마 안된 ETF이다.
    if len(df) < Avg_Period * 20:

        #10개 이하면 즉 상장된지 10일도 안지났다? 0으로 처리!
        if len(df) < (Avg_Period+1):
            ...
            stock_info['avg_momentum_score'] = avg_momentum_score
...
#평균 모멘텀 스코어가 높은 상위 5개를 구한다.
Stockdata = sorted(MyPortfolioList, key=lambda stock_info: (stock_info['avg_momentum_score']), reverse= True)
...
for stock_info in MyPortfolioList:
    ...
    if Common.CheckStockCodeInList(FinalSelectedList,stock_code) == True:
        ...
        if MomentumScore < 0 and SnP500_ETF != stock_code:
...
for stock_info in MyPortfolioList:
    ...
    if stock_amt > 0:
        ...
        if Common.CheckStockCodeInList(FinalSelectedList,stock_code) == True:
            ...
            #백테스팅상 비중대로 맞춰주는 건 성과가 좋지 않았으니 제외!
            #단 SPY,VOO등 지수만 리밸런싱 하자!
            if stock_code == SnP500_ETF:
                #'''
                #목표한 비중가 다르다면!!
