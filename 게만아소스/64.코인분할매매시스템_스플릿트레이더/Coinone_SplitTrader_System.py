...
auto_order_file_path = "/var/autobot/Coinone_SplitTrader_AutoOrderList.json"
...
for AutoSplitData in AutoOrderList:
    ...
    if AutoSplitData['OrderType'] == "Sell":
        ...
        if AutoSplitData['TimeCnt'] >= AutoSplitData['TimeTerm']:
            ...
            nowPrice = myCoinone.GetCurrentPrice(AutoSplitData['Ticker'])
            time.sleep(0.2)
            AllVolume = myCoinone.GetCoinAmount(balances,AutoSplitData['Ticker']) #현재 보유 수량
            time.sleep(0.2)
            ...
            if AllVolume * nowPrice < minimumMoney:
                ...
                #매도!
                balances = myCoinone.SellCoinMarket(AutoSplitData['Ticker'], SellVolume)
                time.sleep(0.2)
    ...
    if AutoSplitData['OrderType'] == "Buy":
        ...
        if AutoSplitData['TimeCnt'] >= AutoSplitData['TimeTerm']:
            ...
            if AutoSplitData['SplitBuyMoney'] < minimumMoney:
                ...
                balances = myCoinone.BuyCoinMarket(AutoSplitData['Ticker'], AutoSplitData['SplitBuyMoney'])
                time.sleep(0.2)
