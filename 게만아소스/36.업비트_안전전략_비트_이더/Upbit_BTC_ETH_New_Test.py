...
##########################################################################################################
    ...
    df = pyupbit.get_ohlcv(coin_ticker,interval="day",count=1619, period=0.3) #day/minute1/minute3/minute5/minute10/minute15/minute30/minute60/minute240/week/month
    print(len(df))
    ...
    for i in range(len(df)):
        ...
        if IsBuy == False and i > 2 and IsSellToday == False: 
            ...
            #시가와 종가의 갭이 고가와 저가의 갭의 50% 이하라면 도지 캔들로 판단
