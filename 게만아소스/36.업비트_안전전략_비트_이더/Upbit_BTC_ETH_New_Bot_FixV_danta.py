...
for coin_data in InvestCoinList:
    ...
    if BotDataDict.get(coin_ticker+"_HAS") == None:
        ...
        #이 부분은 이전 버전의 봇과 호환을 위해 이전에 저장된 파일이 있다면 그것을 읽어서 그 내용을 새로운 파일에 써주는 부분!!!
...
##########################################################################################################
    ...
    ############################# DANTA #############################
    '''
    어디까지나 예시이며 이 단타 로직은 정말 대충 생가나는대로 만든 것입니다.
    여러분만의 단타 전략이 있다면 이렇게 응용할 수 있다는 참고 예시로 체크하세요 ^^
    '''
    #####추가 단타 및 매수 파트#####
    NowMoney = myUpbit.GetCoinNowRealMoney(balances,coin_ticker)
    TargetTotalMoney = InvestTotalMoney * (coin_data['rate'] + coin_data['fix_rate'])
        ...
        if min_n % 15 == 0:
            ...
            if IsBuyGo == True:
                ...
                if InvestAddMoney > won:
                    InvestAddMoney = won * 0.99 #수수료 및 슬리피지 고려
