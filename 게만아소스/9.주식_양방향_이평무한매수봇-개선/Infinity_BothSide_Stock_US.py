...
LongStockCode = ''
ShortStockCode = ''
# LongStockCode = 'TQQQ' #예시 롱 종목 코드
# ShortStockCode = 'SQQQ' #예시 숏 종목 코드

#모멘텀 계산할 기준되는 종목
StStockCode = ""
#StStockCode = "QQQ" #예시 기준 종목 코드
...
if IsMarketOpen == True:
    ...
    #파일 경로입니다.
    status_file_path = "/var/autobot/UsStock_" + BOT_NAME + "_MomentumDataInfo.json"
    try:
    ...
    if Calc_Momentum == True:
        ...
        df = Common.GetOhlcv("US",StStockCode)
                ...
                if IsSellAll == True:
                    ...
                    if stock_amt > 0 and StockInfo['Round'] > 0:
                        ...
                        if ENABLE_ORDER_EXECUTION == True:
                            ...
                            msg = stock_code + " 이평무한매수봇 모두 팔아서 손익확정!!!!  [" + str(stock_revenue_money) + "] 확정 조으다! (현재 [" + str(StockInfo['Round']) + "] 라운드까지 진행되었고 모든 수량 매도 처리! )"
                            print(msg) 
                            line_alert.SendMessage(msg) 
                    ...
                    if StockInfo['IsReady'] == 'Y':
                        ...
                        if IsBuyGo == True:
                            ...
                            if ENABLE_ORDER_EXECUTION == True:
                                ...
                                pprint.pprint(KisUS.MakeBuyLimitOrder(stock_code,int(BuyAmt),CurrentPrice*1.01))
