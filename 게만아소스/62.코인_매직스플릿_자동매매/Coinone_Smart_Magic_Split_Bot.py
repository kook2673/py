        ...
        if MagicDataInfo['coin_ticker'] == coin_ticker:
                ...
                if MagicData['Number'] == 1: #1차수를 찾아서!
                    ...
                    if MagicData['IsBuy'] == False and MagicDataInfo['Date'] != day_n: #매수하지 않은 상태라면 매수를 진행한다!
                        ...
                        if prevOpen < prevClose and (prevClose >= Ma5 or Ma5_Before <= Ma5):
                            ...
                            if myCoinone.IsHasCoin(balances,coin_ticker) == True and myCoinone.GetCoinNowRealMoney(balances,coin_ticker) >= 5000:
                    ...
                    if myCoinone.IsHasCoin(balances,coin_ticker) == False and myCoinone.GetCoinNowRealMoney(balances,coin_ticker) < 5000: #잔고가 0이라면 2차이후의 차수 매매는 없는거니깐 초기화!
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                ...
                if MagicData['IsBuy'] == True:
                    ...
                    if CurrentRate >= PickSplitMeta['target_rate'] and myCoinone.IsHasCoin(balances,coin_ticker) == True and myCoinone.GetCoinNowRealMoney(balances,coin_ticker) >= 5000 :#and (revenue_data['revenue_money'] + MagicDataInfo['RealizedPNL']) > 0 :
