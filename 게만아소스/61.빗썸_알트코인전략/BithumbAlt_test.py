...
combined_df = pd.read_csv('./BithumbDataAlt.csv', index_col=0)
...
for date in combined_df.index.unique():
    ...
    pick_coins_top = combined_df.loc[combined_df.index == date].groupby('ticker')['prevValue'].max().nlargest(50).nsmallest((int(DivNum))) #거래대금 상위 50개 중 하위 30개

    pick_coins_top_change = combined_df.loc[combined_df.index == date].groupby('ticker')['prevChange'].max().nlargest(50).nsmallest((int(DivNum))) #등락률 상위 50개 중 하위 30개
    ...
    for ticker in pick_coins_top.index:
        ...
        if ticker in ['KRW-USDT','KRW-BTC','KRW-ETH']: #비트코인과 이더리움 그리고 테더는 제외!
            continue
