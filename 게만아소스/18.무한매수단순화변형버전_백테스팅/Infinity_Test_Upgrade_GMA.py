...
for stock_code in InvestStockList:
    ...
    df['107ma'] = df['close'].rolling(107).mean() # 개선된 점 GMA
        ...
        if IsBuy == True:
            ...
            if (TakePrice <= NowOpenPrice) or BuyCnt >= DivNum or IsNoCash == True:
                ...
                if BuyCnt < DivNum:
                    ...
                    if df['107ma'].iloc[i-1] > df['close'].iloc[i-1]: # 개선된 점 GMA  107일선 기준으로!!

                        if df['3ma'].iloc[i-2] < df['3ma'].iloc[i-1]: # 개선된 점 GMA  #전일까지 3일선이 증가했다면 그때만 매수!!
                            IsBuyGo = True
