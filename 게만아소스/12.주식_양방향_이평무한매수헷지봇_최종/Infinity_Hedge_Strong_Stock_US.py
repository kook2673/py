...
BOT_NAME = Common.GetNowDist() + "_InfinityHedgeStBot"
...
#투자할 종목!
TargetStockList = ['']
#TargetStockList = ['TQQQ']  #예시


InverseShortCode = dict()
InverseShortCode[''] = '' # [롱] = 숏
#InverseShortCode['TQQQ'] = 'SQQQ' #예시
...
if ENABLE_ORDER_EXECUTION == True:
    ...
    if IsMarketOpen == True:
        ...
        for stock_code in TargetStockList:
            ...
            #현재가
            Short_CurrentPrice = KisUS.GetCurrentPrice(ShortCode)
            ...
            if PickStockInfo == None:
                ...
                if stock_amt == 0:
                    ...
                    InfinityDataDict['Inverse_Hedge_WaterAmt'] = 0 #헤지 모드로 들어간 수량!
                ...
                if StockInfo['StockCode'] == stock_code:
                    ...
                    if StockInfo['Round'] > 0 and stock_amt >= 1:
                        ...
                        if StockInfo['IsTralling'] == 'Y':
                            ...
                            if CurrentPrice <= StopPrice:
                                ...
                                StockInfo['Inverse_S_WaterAmt'] = 0 #롱 물탈때 같이 탄 수량!
                                StockInfo['Inverse_Hedge_WaterAmt'] = 0 #헤지 모드로 들어간 수량!
                        ...
                        #트레일링 스탑 아직 시작 안됨!
                            ...
                            print("목표 수익률 : ", round(FinalRate*100.0,2) ,"% 현재 ",stock_code," 수익률 : " ,  stock_revenue_rate, "%")
                            #수익화할 가격을 구한다!
                            ...
                            if CurrentPrice >= RevenuePrice and (stock_revenue_money + Short_stock_revenue_money) > 0:
                                ...
                                if Short_stock_amt >= 1:
                                    ...
                                    #현재가보다 아래에 매도 주문을 넣음으로써 시장가로 매도효과!
                                    pprint.pprint(KisUS.MakeSellLimitOrder(ShortCode,Short_stock_amt,Short_CurrentPrice*0.99))
            

                                    msg = ShortCode + " 이평무한매수봇 숏 모두 정리!!!!  [" + str(Short_stock_revenue_money) + "] 손익 확정!"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 
                                ...
                                if stock_amt == 1:
                                    ...
                                    #절반만 팝니다!!!!!
                                    pprint.pprint(KisUS.MakeSellLimitOrder(stock_code,HalfAmt,CurrentPrice*0.99))


                                    msg = stock_code + " 이평무한매수봇 절반 팔아서 수익확정!!!!  [" + str(stock_revenue_money*0.5) + "] 수익 조으다! (현재 [" + str(StockInfo['Round']) + "] 라운드까지 진행되었고 절반 익절 후 트레일링 스탑 시작!!)"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 
                    ...
                    if StockInfo['IsReady'] == 'Y' and StockInfo['IsTralling'] =='N':
                        ...
                        if CurrentPrice > StMoney:
                            ...
                            msg = stock_code + " 현재 60분할된 투자금보다 현재가가 높아서 전략을 제대로 수행 못해요!  자금을 투자비중이나늘리세요!"
                            print(msg)   
                            line_alert.SendMessage(msg) 
                        ...
                        #######################################################################
                        if Ma20 > CurrentPrice: #20선 밑에 있다면 1/4의 금액만큼 헷징들어간다!

                            if StockInfo['Inverse_Hedge_WaterAmt'] == 0:

                                BuyRound = int(StockInfo['Round']/4) + 1 #물탈 회수

                                ###########################################################################################
                                ShortBuyAmt = int((StMoney * BuyRound) / Short_CurrentPrice)
                                
                                if ShortBuyAmt < 1:
                                    ShortBuyAmt = 1

                                AbleAmt = (int((StMoney * 10) / Short_CurrentPrice)) - StockInfo['Inverse_S_WaterAmt']

                                if AbleAmt < ShortBuyAmt:
                                    ShortBuyAmt = AbleAmt


                                StockInfo['Inverse_Hedge_WaterAmt'] = ShortBuyAmt

                                #시장가 주문을 넣는다!
                                pprint.pprint(KisUS.MakeBuyLimitOrder(ShortCode,ShortBuyAmt,Short_CurrentPrice*1.01))

                                ###########################################################################################
                            
                                
                        else:
                            if StockInfo['Inverse_Hedge_WaterAmt'] != 0 and Ma20 < CurrentPrice:
                                #현재가보다 아래에 매도 주문을 넣음으로써 시장가로 매도효과!
                                pprint.pprint(KisUS.MakeSellLimitOrder(ShortCode,StockInfo['Inverse_Hedge_WaterAmt'],CurrentPrice*0.99))

                                #단 현재 수익금이 마이너스 즉 손실 상태라면 손실 금액을 저장해 둔다!
                                if Short_stock_revenue_money < 0:
                                    #손실 금액에서 매도수량/보유수량 즉 예로 10개보유 하고 현재 -100달러인데 3개를 파는 거라면 실제 확정 손실금은 -100 * (3/10)이 니깐~
                                    LossMoney = abs(Short_stock_revenue_money) * (float(StockInfo['Inverse_Hedge_WaterAmt']) / float(Short_stock_amt))
                                    StockInfo['S_WaterLossMoney'] += LossMoney

                                    msg = ShortCode + "  20선 위로 올라와서 숏 매도합니다! 약 [" + str(LossMoney) + "] 확정손실이 나서 기록했으며 누적 손실은 [" + str(StockInfo['S_WaterLossMoney']) + "] 입니다"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 
                                ...
                                #이 경우는 이득 본 경우다!
                                    ...
                                    #이득본 금액도 계산해보자
                                    RevenuMoney = abs(Short_stock_revenue_money) * (float(StockInfo['Inverse_Hedge_WaterAmt']) / float(Short_stock_amt))
                                    ...
                                    msg = ShortCode + "  20선 위로 올라와서 숏 매도합니다! 약 [" + str(RevenuMoney) + "] 확정 수익이 났네요! 누적 손실은 [" + str(StockInfo['S_WaterLossMoney']) + "] 입니다"
                                    print(msg) 
                                    line_alert.SendMessage(msg) 

                                StockInfo['Inverse_Hedge_WaterAmt'] = 0 #팔았으니 0으로 초기화!
                        ...
                        if StockInfo['S_WaterAmt'] != 0:
                            ...
                            #현재가보다 아래에 매도 주문을 넣음으로써 시장가로 매도효과!
                            pprint.pprint(KisUS.MakeSellLimitOrder(stock_code,StockInfo['S_WaterAmt'],CurrentPrice*0.99))
                            ...
                            #이 경우는 이득 본 경우다!
                                ...
                                msg = stock_code + " 평단을 확 낮추기 위한 이평무한매수봇 물탄지 하루가 지나 그 수량만큼 매도합니다! 약 [" + str(RevenuMoney) + "] 확정 수익이 났네요! 누적 손실은 [" + str(StockInfo['S_WaterLossMoney']) + "] 입니다"
                                print(msg) 
                                line_alert.SendMessage(msg) 
                            ...
                            ###########################################################################################
                                                    #현재가보다 아래에 매도 주문을 넣음으로써 시장가로 매도효과!
                            pprint.pprint(KisUS.MakeSellLimitOrder(ShortCode,StockInfo['Inverse_S_WaterAmt'],CurrentPrice*0.99))
                            ...
                            if Short_stock_revenue_money < 0:
                                ...
                                msg = ShortCode + " 평단을 확 낮추기 위한 이평무한매수봇 물탄지 하루가 지나 인버스(숏)  수량만큼 매도합니다! 약 [" + str(LossMoney) + "] 확정손실이 나서 기록했으며 누적 손실은 [" + str(StockInfo['S_WaterLossMoney']) + "] 입니다"
                                print(msg) 
                                line_alert.SendMessage(msg) 
                            ...
                            #이 경우는 이득 본 경우다!
                                ...
                                msg = ShortCode + " 평단을 확 낮추기 위한 이평무한매수봇 물탄지 하루가 지나 인버스(숏)  수량만큼 매도합니다! 약 [" + str(RevenuMoney) + "] 확정 수익이 났네요! 누적 손실은 [" + str(StockInfo['S_WaterLossMoney']) + "] 입니다"
                                print(msg) 
                                line_alert.SendMessage(msg) 
                        ...
                        #평단 낮추기위한 물타기 미진행!
                            ...
                            if Ma5 < Ma20 and Ma5_before3 > Ma5_before and Ma5_before < Ma5:
                                ...
                                AbleAmt = (int((StMoney * 10) / Short_CurrentPrice)) - StockInfo['Inverse_Hedge_WaterAmt']
