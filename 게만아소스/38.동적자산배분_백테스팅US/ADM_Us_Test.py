...
InvestStockList = ["SPY","SCZ","TLT","TIP"]
...
for stock_code in InvestStockList:
    ...
    df['Momentum'] = ( ((df['prevClose'] - df['1month'])/df['1month'])  + ((df['prevClose'] - df['3month'])/df['3month'])  + ((df['prevClose'] - df['6month'])/df['6month']) ) / 3.0

    df['Momentum_1'] = ((df['prevClose'] - df['1month'])/df['1month']) 
...
for date in combined_df.index.unique():
    ...
    scz_data = combined_df[(combined_df.index == date) & (combined_df['stock_code'] == "SCZ")] 



    pick_atk_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "TLT") & (combined_df['stock_code'] != "TIP")].groupby('stock_code')['Momentum'].max().nlargest(1)


    pick_safe_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "SPY") & (combined_df['stock_code'] != "SCZ")].groupby('stock_code')['Momentum_1'].max().nlargest(1)
    ...
    if len(checkall) == len(NowInvestList) and int(date_object.strftime("%Y")) >= StartYear:
        ...
        if spy_data['Momentum'].values[0] > 0 and scz_data['Momentum'].values[0] > 0  :
