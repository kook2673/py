...
InvestStockList = ["SPY","IWM","QQQ","VGK","EWJ","EEM","VNQ","DBC","GLD","TLT","HYG","LQD","SHY","IEF","AGG"]
...
for date in combined_df.index.unique():
    ...
    pick_atk_momentum_top1 = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "AGG") & (combined_df['stock_code'] != "SHY") & (combined_df['stock_code'] != "IEF")].groupby('stock_code')['Momentum'].max().nlargest(1)
    pick_atk_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "AGG") & (combined_df['stock_code'] != "SHY") & (combined_df['stock_code'] != "IEF")].groupby('stock_code')['Momentum'].max().nlargest(6)


    pick_safe_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "AGG") & (combined_df['stock_code'] != "HYG") & (combined_df['stock_code'] != "TLT")  & (combined_df['stock_code'] != "GLD") & (combined_df['stock_code'] != "DBC")  & (combined_df['stock_code'] != "VNQ") & (combined_df['stock_code'] != "EEM") & (combined_df['stock_code'] != "EWJ") & (combined_df['stock_code'] != "SPY") & (combined_df['stock_code'] != "IWM") & (combined_df['stock_code'] != "QQQ") & (combined_df['stock_code'] != "VGK")].groupby('stock_code')['Momentum'].max().nlargest(1)
    ...
    if len(checkall) == len(NowInvestList) and int(date_object.strftime("%Y")) >= StartYear:
        ...
        #카나리아 자산 모두 모멘텀이 0보다 크다!
        if eem_data['Momentum'].values[0] > 0 and agg_data['Momentum'].values[0] > 0 :
        ...
        elif eem_data['Momentum'].values[0] > 0 or agg_data['Momentum'].values[0] > 0 :
            ...
            if investData['IsRebalanceGo'] == True:
                ...
                if len(stock_data) == 1:
                    ...
                    if stock_code in AtkOkList:
                        ...
                        investData['InvestRate'] += ((1.0 - SafeRate) / len(AtkOkList))
