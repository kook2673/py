...
InvestStockList = ["SPY","IWM","VWO","VEA","TLT","IEF","DBC","VNQ","TIP","BIL"]
...
for date in combined_df.index.unique():
    ...
    ief_data = combined_df[(combined_df.index == date) & (combined_df['stock_code'] == "IEF")] 

    pick_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "TIP") & (combined_df['stock_code'] != "BIL")].groupby('stock_code')['Momentum'].max().nlargest(4)

    checkall = combined_df.loc[(combined_df.index == date)].groupby('stock_code')['Momentum'].max().nlargest(len(NowInvestList))


    if len(tip_data) == 1 and len(bil_data) == 1 and len(ief_data) == 1 and len(checkall) == len(NowInvestList) and int(date_object.strftime("%Y")) >= StartYear:
            ...
            if investData['IsRebalanceGo'] == True:
                ...
                if len(stock_data) == 1:
                    ...
                    if SafeRate > 0:
                        ...
                        #IEF가 더 모멘텀이 높다면 IEF에!!
                        if ief_data['Momentum'].values[0] > bil_data['Momentum'].values[0]:
                        ...
                        #BIL이 더 모멘텀이 높다면 BIL에!!
                        elif ief_data['Momentum'].values[0] < bil_data['Momentum'].values[0]:
