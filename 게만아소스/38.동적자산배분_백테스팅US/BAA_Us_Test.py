...
InvestStockList = ["QQQ","VWO","VEA","BND","TIP","DBC","BIL","IEF","TLT","LQD","SPY"]
...
for stock_code in InvestStockList:
    ...
    df['240ma'] = df['close'].rolling(240).mean().shift(1) 

    df['Momentum_12'] = df['prevClose'] / df['240ma']
...
for date in combined_df.index.unique():
    ...
    bnd_data = combined_df[(combined_df.index == date) & (combined_df['stock_code'] == "BND")] 



    pick_atk_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "LQD") & (combined_df['stock_code'] != "TLT") & (combined_df['stock_code'] != "IEF") & (combined_df['stock_code'] != "BIL") & (combined_df['stock_code'] != "DBC") & (combined_df['stock_code'] != "SPY") & (combined_df['stock_code'] != "TIP")].groupby('stock_code')['Momentum_12'].max().nlargest(1)


    pick_safe_momentum_top = combined_df.loc[(combined_df.index == date) & (combined_df['stock_code'] != "QQQ") & (combined_df['stock_code'] != "VWO") & (combined_df['stock_code'] != "VEA") & (combined_df['stock_code'] != "SPY")].groupby('stock_code')['Momentum_12'].max().nlargest(3)
    ...
    if len(checkall) == len(NowInvestList) and int(date_object.strftime("%Y")) >= StartYear:
        ...
        if spy_data['Momentum'].values[0] > 0 and vwo_data['Momentum'].values[0] > 0  and vea_data['Momentum'].values[0] > 0  and bnd_data['Momentum'].values[0] > 0 :
                    ...
                    if SafeRate > 0:
                        ...
                        for stock_code_b in pick_safe_momentum_top.index:
                            ...
                            if stock_code_b == stock_code:
                                ...
                                if stock_data['240ma'].values[0] <= stock_data['prevClose'].values[0]:
