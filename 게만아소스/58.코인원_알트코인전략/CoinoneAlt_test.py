...
MakeCoinoneDataForAltTest.py 파일로 데이터 수집을 먼저 해야 백테스팅이 진행 됩니다!!!
...
combined_df = pd.read_csv('./CoinoneDataHalt.csv', index_col=0)
...
for date in combined_df.index.unique():
    ...
    #eth_data = combined_df[(combined_df.index == date) & (combined_df['ticker'] == 'ETH')]
