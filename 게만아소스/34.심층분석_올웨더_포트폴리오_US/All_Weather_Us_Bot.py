...
StockCodeList = [] #투자 종목을 아래 예시처럼 직접 판단해서 넣으세요!
#StockCodeList = ['IYK','SPY','TLT','IEF','GLD','PDBC'] #예시

################################################################

InvestType = 'A' #자산가형 
#InvestType = 'B' #초심자 공격형
...
for stock_code in StockCodeList:
    ...
    if stock_code == "TLT":
        asset['stock_target_rate'] = 27
        asset['stock_type'] = "BOND" #채권(RISK)인지 주식(STOCK)인지 대체 자산(ALT)인지 
    elif stock_code == "IEF":
        ...
        asset['stock_type'] = "STOCK"  #채권(RISK)인지 주식(STOCK)인지 대체 자산(ALT)인지 
    elif stock_code == "IYK":
        asset['stock_target_rate'] = 22
        asset['stock_type'] = "STOCK" 
        
    elif stock_code == "GLD":
        asset['stock_target_rate'] = 8
        asset['stock_type'] = "ALT"  #채권(RISK)인지 주식(STOCK)인지 대체 자산(ALT)인지 
    elif stock_code == "PDBC":
        asset['stock_target_rate'] = 8
        asset['stock_type'] = "ALT" 
...
BOT_NAME = Common.GetNowDist() + "_MyAllWeatherBotUs"
...
print("date_check", now_time, now_time.month )

#5~10월은 지옥이다!!
if 5 <= now_time.month <= 10:
    IsHell = True

#현재 천국 기간이어도
if IsHell == False:
    spy_df = Common.GetOhlcv("US","SPY")

    SPYprevClose = spy_df['close'].iloc[-2]
    SPYMa50 = Common.GetMA(spy_df,50,-2) 
    SPYMa150 = Common.GetMA(spy_df,150,-2)

    #150,50선 밑에 있다면 지옥기간!!
    if SPYMa150 > SPYprevClose and SPYMa50 > SPYprevClose:
        IsHell = True
...
for stock_info in MyPortfolioList:
    ...
    Ma120_Before = Common.GetMA(df,120,-3)
    Ma120 = Common.GetMA(df,120,-2)

    Ma150 = Common.GetMA(df,150,-2)
    ...
    print(stock_code," -> 투자 비중: ", stock_info['stock_target_rate'], "%")
...
for stock_info in MyPortfolioList:
    ...
    #잔고에 없는 경우
        ...
        if InvestType == 'A':
            #자산가형은 일단 무조건 1주 이상 사주자... 아니라면 아래 2줄 주석처리
            if BuyAmt <= 0:
                BuyAmt = 1
