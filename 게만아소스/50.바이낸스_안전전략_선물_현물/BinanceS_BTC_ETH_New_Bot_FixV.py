...
import my_key    #바이낸스(현물) 시크릿 액세스키
...
#파일 경로입니다.
botdata_file_path = "/var/autobot/BinanceS_Data.json"
try:
...
for coin_data in InvestCoinList:
    ...
    ##########################################################################################################
        ...
        #if abs(amt_b) != 0: 
        ############################# FixV ####################################
        if BotDataDict[coin_ticker+"_HAS"] == True and Target_Coin_Ticker in HasCoinList:
            print("전략으로 매수한 경우!")
            ...
            if BotDataDict[coin_ticker+"_DATE_CHECK"] != day_n:
                ...
                msg = coin_ticker + "현물 " + str(coin_amt)+ "개 현재 보유중..."
                print(msg)
            ...
            if IsSellGo == True:
                ...
                NowMoney = NowCurrentPrice * coin_amt
                ...
                SellAmt = float(binanceX.amount_to_precision(Target_Coin_Ticker, myBinance.GetAmount(SellMoney,NowCurrentPrice,1.0)))
                print("SellAmt: ", SellAmt)


                if SellMoney == 0:
                    print("고정비중보다 현재 적은 상황이니 매도하지 않고 처리만 해준다!")
                
                else:

                    if SellMoney >= minimun_usdt:
                        data = binanceX.create_order(Target_Coin_Ticker, 'market', 'sell', SellAmt)
                        

                if IsDolpaCut == True:
                    msg = coin_ticker + " 바이낸스(현물) 안전 전략 봇 : 조건을 하향 돌파 불만족하여 모두 매도처리 했어요!! "
                    print(msg)
                    line_alert.SendMessage(msg)
                else:
                                
                    msg = coin_ticker + " 바이낸스(현물) 안전 전략 봇 : 조건을 불만족하여 모두 매도처리 했어요!! s"
                    print(msg)
                    line_alert.SendMessage(msg)
            ...
            if IsMaDone == True :
                ...
                Buy_Amt = float(binanceX.amount_to_precision(Target_Coin_Ticker, myBinance.GetAmount(BuyMoney,NowCurrentPrice,1.0))) 
                ...
                msg = coin_ticker + " 바이낸스(현물) 안전 전략 봇 :  조건 만족 하여 매수!! "
                ...
                if hour_n == 0 and min_n == 0:
                    msg = coin_ticker + " 바이낸스(현물) 안전 전략 봇 :  조건 만족하지 않아 현금 보유 합니다!"
                    print(msg)
                    line_alert.SendMessage(msg)
        ...
        if coin_data['fix_rate'] > 0:
            ...
            if Target_Coin_Ticker not in HasCoinList: 
                ...
                msg = coin_ticker + " 바이낸스(현물) 안전 전략 봇 :  고정비중이 없어 투자!!"
                print(msg)
                line_alert.SendMessage(msg)
                ...
                if Fix_Rebalance_Enable == True and BotDataDict[coin_ticker+"_HAS"] == False:
                    ...
                    #현재 코인의 총 매수금액
                    NowCoinTotalMoney = NowCurrentPrice * coin_amt
                    ...
                    if Rate != coin_data['fix_rate']:
                        ...
                        print("GapMoney ", GapMoney)
            
                        
                        if GapMoney >= minimun_usdt*2.0:
                            
                            GapAmt = float(binanceX.amount_to_precision(Target_Coin_Ticker, myBinance.GetAmount(GapMoney,NowCurrentPrice,1.0))) 
                            ...
                            if GapRate < 0:
                                ...
                                if abs(GapRate) >= (coin_data['fix_rate'] / 10.0): 
        

                                    #롱 포지션 종료!!
                                    data = binanceX.create_order(Target_Coin_Ticker, 'market', 'sell', GapAmt)

                                    line_alert.SendMessage(coin_ticker + " 바이낸스(현물) 안전 전략 봇 : 리밸런싱 진행!!!! 일부 매도!" )
                            ...
                            #갭이 양수면 비트코인 비중이 적으니 추매할 필요가 있는 경우
                                ...
                                if abs(GapRate) >= (coin_data['fix_rate'] / 10.0):
                                    ...
                                    #매수!
                                    data = binanceX.create_order(Target_Coin_Ticker, 'market', 'buy', GapAmt)
                                    
                                    line_alert.SendMessage(coin_ticker + " 바이낸스(현물) 안전 전략 봇 : 리밸런싱 진행!!! 일부 매수!" )
