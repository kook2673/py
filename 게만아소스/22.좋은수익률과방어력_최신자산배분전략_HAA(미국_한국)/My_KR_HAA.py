...
RiskCodeList = [] #아래 예시처럼 직접 종목을 채우세요
#RiskCodeList = ['360750','280930','251350','195980','304660','305080','276000','352560']
...
SafeCodeList = []#아래 예시처럼 직접 종목을 채우세요
#SafeCodeList = ['305080','BIL'] #BIL에 투자하진 않지만 현금 모멘텀 계산을 위해 
...
BirdCodeList = [] #아래 예시처럼 직접 종목을 채우세요
#BirdCodeList = ['TIP'] #KOSEF 물가채 KIS "430500" 이걸로 해도 비슷?
...
BOT_NAME = Common.GetNowDist() + "_MyHAABotKR"
...
for stock_info in MyPortfolioList:
    ...
    if stock_code == "BIL" or stock_code == "TIP":
...
#안전 자산 중 가장 모멘텀 스코어가 높은거를 SafeAssetCnt개 만큼 순서대로 구한다! - TIGER 미국채10년선물 '305080'  혹은 BIL
...
for stock_info in MyPortfolioList:
    ...
    if GoAttack == True:
        ...
        if Is_Safe_Add_Rate == 0:
            #위험자산에 IEF(305080)가 속해 있다면 비중에 따라 매수매도가 될테니깐 안전자산 체크시 매도를 피합니다!
            if '305080' in RiskTopStockCodeList and stock_info['stock_type'] == "SAFE" and stock_code == "305080":
                continue

    #안전자산에 투자 해야 하는 상태
    else:
        #안전자산에 IEF(305080)가 속해 있다면 비중에 따라 매수매도가 될테니깐 위험자산 체크시 매도를 피합니다!
        if '305080' in SafeTopStockCodeList and stock_info['stock_type'] == "RISK" and stock_code == "305080":
            continue
