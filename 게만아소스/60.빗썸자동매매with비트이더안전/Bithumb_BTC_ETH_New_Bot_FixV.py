...
##########################################################################################################
    ...
    ############################# FixV ####################################
    if BotDataDict[coin_ticker+"_HAS"] == True and myBithumb.IsHasCoin(balances,coin_ticker) == True and myBithumb.GetCoinNowRealMoney(balances,coin_ticker) >= minmunMoney:
        print("전략으로 매수한 경우!")
    ...
    if coin_data['fix_rate'] > 0:
        ...
        if myBithumb.IsHasCoin(balances,coin_ticker) == False or myBithumb.GetCoinNowRealMoney(balances,coin_ticker) < minmunMoney: 
