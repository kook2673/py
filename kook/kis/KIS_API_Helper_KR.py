...
def MarketStatus(stock_code = '069500'):
    ...
    print(f"=== MarketStatus API Call Debug ===")
    print(f"URL: {URL}")
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if response_json["rt_cd"] == '0':
                ...
                print(f"API Error - rt_cd: {response_json.get('rt_cd')}, msg: {response_json.get('msg')}")
                return response_json.get("msg_cd", "Unknown error")
        else:
        ...
        print(f"Exception in MarketStatus: {e}")
        print(f"Exception type: {type(e)}")
        import traceback
        traceback.print_exc()
        return f"Exception: {str(e)}"
    
    print("=== End MarketStatus API Call Debug ===")
...
def IsMarketOpen():
    # print("=== IsMarketOpen Function Start ===")
    # print(f"Current working directory: {os.getcwd()}")
    # print(f"Script directory: {os.path.dirname(os.path.abspath(__file__))}")
    # print(f"Environment variables:")
    # for key, value in os.environ.items():
    #     if 'PATH' in key or 'PYTHON' in key or 'HOME' in key:
    #         print(f"  {key}: {value}")
    ...
    if IsOpen == True:
        ...
        file_path = os.path.join(script_dir, "KR_Market_OpenCheck.json")
        try:
        ...
        if Is_CheckTody == True:
            ...
            market = MarketStatus()
            print("=== MarketStatus Debug Info ===")
            print(f"market type: {type(market)}")
            print(f"market value: {market}")
            print(f"market repr: {repr(market)}")
            pprint.pprint(market)
            print("=== End MarketStatus Debug Info ===")

            IsJangJung = False
            if isinstance(market, dict) and 'Status' in market:
                if (market['Status'] == '2'):
                    IsJangJung = True
            else:
                print(f"ERROR: market is not a dict or missing 'Status' key. market: {market}")
                IsJangJung = False
            ...
            if IsTodayOpenCheck() == 'N' or IsJangJung == False:
                # 간헐적 오탐 방지를 위해 한 번 더 재확인 (2초 대기 후 재조회)
                try:
                    time.sleep(2)
                except Exception:
                    pass
                ...
                market = MarketStatus()
                IsJangJung_retry = False
                if isinstance(market, dict) and 'Status' in market:
                    if (market['Status'] == '2'):
                        IsJangJung_retry = True

                if IsTodayOpenCheck() == 'N' or IsJangJung_retry == False:
                    print("Market is Close!!")
                    return False
                else:
                    print("Market is Open (Retry)!!!!")
            #아니라면 열린거다
            else:
...
def MakeBuyMarketOrder(stockcode, amt, adjustAmt = False):
    ...
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        ...
        data = {
            ...
            "ORD_QTY": str(int(amt)),
            "ORD_UNPR": "0",
            "EXCG_ID_DVSN_CD": "KRX"  # 거래소 구분 코드 추가
            #"CNDT_PRIC": "0"  # 조건가격 추가
        }
        headers = {"Content-Type":"application/json", 
            "authorization":f"Bearer {Common.GetToken(Common.GetNowDist())}",
...
#지정가 주문하기!
def MakeBuyLimitOrder(stockcode, amt, price, adjustAmt = False, ErrLog = "NO", EXCG_ID_DVSN_CD = "KRX"):
    ...
    if int(Common.GetPrdtNo(Common.GetNowDist())) == 29:
        ...
        data = {
            ...
            "EXCG_ID_DVSN_CD": EXCG_ID_DVSN_CD  # 거래소 구분 코드 (KRX/NXT/SOR)
            #"CNDT_PRIC": "0"  # 조건가격 추가
        }
        headers = {"Content-Type":"application/json", 
            "authorization":f"Bearer {Common.GetToken(Common.GetNowDist())}",
...
#지정가 매도하기!
def MakeSellLimitOrder(stockcode, amt, price, ErrLog="YES", EXCG_ID_DVSN_CD = "KRX"):
...
#당일 분봉 조회!
def GetOhlcvMinute(stock_code, MinSt = '1T', get_count = 500):
    ...
    while DataLoad:
        ...
        headers = {"Content-Type":"application/json", 
                ...
                "appKey":Common.GetAppKey(Common.GetNowDist()),
                "appSecret":Common.GetAppSecret(Common.GetNowDist()),
                "tr_id":"FHKST03010200"} # F제거. 25.03.03 F다시 넣음
    ...
    try:
        ...
        for stock in stock_list:
            if stock['StockCode'] == stock_code:
                return {
                    'quantity': int(stock['StockAmt']),
                    'current_price': float(stock['StockNowPrice']),
                    'avg_price': float(stock['StockAvgPrice']),
                    'profit_rate': float(stock['StockRevenueRate']),
                    'total_value': float(stock['StockNowMoney']),
                    'profit_amount': float(stock['StockRevenueMoney'])
                }
...
#토큰 만료 오류를 감지하고 자동으로 재발급받는 API 호출 래퍼 함수
def CallAPIWithTokenRefresh(api_func, *args, **kwargs):
    """
    API 함수를 호출하되, 토큰 만료 시 자동으로 재발급받는 래퍼 함수
    """
    try:
        #첫 번째 시도
        ...
        error_msg = str(e)
        #토큰 만료 오류 감지
        if "EGW00123" in error_msg or "기간이 만료된 token" in error_msg or "만료된 token" in error_msg:
            print("토큰이 만료되었습니다. 새로운 토큰을 발급받습니다...")
            #토큰 재발급
            ...
            try:
                ...
                print(f"토큰 재발급 후에도 API 호출 실패: {e2}")
                return None
        else:
            #다른 오류인 경우 그대로 전파
            raise e
