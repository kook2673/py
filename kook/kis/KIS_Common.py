...
#설정 파일 정보를 읽어 옵니다.
# 현재 스크립트 파일의 디렉토리를 기준으로 절대경로 설정
script_dir = os.path.dirname(os.path.abspath(__file__))
config_file_path = os.path.join(script_dir, 'myStockInfo.yaml')
...
def GetTokenPath(dist = "REAL"):
    ...
    # 설정 파일에서 토큰 파일명만 가져와서 현재 디렉토리에 경로 설정
    token_filename = stock_info[key]
...
def GetTokenPath(dist = "REAL"):
    ...
    # 설정 파일에서 토큰 파일명만 가져와서 현재 디렉토리에 경로 설정
    token_filename = stock_info[dist + "_TOKEN_PATH"]
    token_path = os.path.join(script_dir, token_filename)
...
#토큰 만료 시 자동으로 재발급받는 함수
def GetTokenWithRefresh(dist = "REAL"):
    """
    토큰을 가져오되, 만료된 경우 자동으로 재발급받는 함수
    """
    return GetToken(dist)

#토큰 만료 오류를 감지하고 자동으로 재발급받는 함수
def RefreshTokenIfExpired(dist = "REAL"):
    """
    토큰이 만료되었는지 확인하고, 만료된 경우 새로운 토큰을 발급받는 함수
    """
    try:
        #기존 토큰 파일을 삭제하여 강제로 새 토큰 발급
        import os
        token_path = GetTokenPath(dist)
        if os.path.exists(token_path):
            os.remove(token_path)
            print("토큰 파일을 삭제하여 새로운 토큰을 발급받습니다.")
        
        return MakeToken(dist)
    except Exception as e:
        print(f"토큰 재발급 중 오류 발생: {e}")
        return None
...
def GetOhlcv(area, stock_code, limit = 500, adj_ok = "1"):
    ...
    try:
        ...
        if area == "US":
            ...
            # 한투 데이터 가져오기 (토큰 만료 시 자동 재발급)
            try:
                df_kis = KisUS.GetOhlcv(stock_code,"D")
                if df_kis is None:
                    print(f"한투 데이터 조회 실패 ({stock_code}). 토큰을 재발급받습니다...")
                    RefreshTokenIfExpired(GetNowDist())
                    df_kis = KisUS.GetOhlcv(stock_code,"D")
            except Exception as e:
                error_msg = str(e)
                if "EGW00123" in error_msg or "기간이 만료된 token" in error_msg:
                    print(f"토큰 만료 오류 발생 ({stock_code}). 토큰을 재발급받습니다...")
                    RefreshTokenIfExpired(GetNowDist())
                    df_kis = KisUS.GetOhlcv(stock_code,"D")
                else:
                    print(f"한투 데이터 조회 중 오류 발생 ({stock_code}): {e}")
                    return None
            
            if df_kis is None or len(df_kis) == 0:
                print(f"한투 데이터를 가져올 수 없습니다 ({stock_code})")
                return None
                
            print("한투 데이터 마지막 날짜:", df_kis.index[-1])
            kis_last_date = pd.to_datetime(df_kis.index[-1])
            ...
            try:
                ...
                print("Yfinance 마지막 날짜:", df.index[-1])
                df_last_date = pd.to_datetime(df.index[-1])

                if kis_last_date > df_last_date:
                    # 한투 데이터에서 Yfinance 마지막 날짜 이후의 데이터만 추출
                    new_data = df_kis[df_kis.index > df.index[-1]]
                    # 데이터프레임 합치기
                    df = pd.concat([df, new_data])
                    print("yfinance 신규 데이터 누락으로 인해 추가 완료!")
                else:
                    print("yfinance 데이터를 잘 가져옴")
            ...
            if except_riase == True:
                ...
                try:
                    ...
                    if kis_last_date > df_last_date:
                        ...
                        # 데이터프레임 합치기
                        df = pd.concat([df, new_data])
                        print("FDR 신규 데이터 누락으로 인해 추가 완료!")
                    else:
                        print("FDR 데이터를 잘 가져옴")
                ...
                if except_riase == True:
                    ...
                    if except_riase == True:
                        print("모두 실패하여 맨 처음 가져온 100개 데이터만 리턴!")
                        df = df_kis
            ...
            # 한투 데이터 가져오기 (토큰 만료 시 자동 재발급)
            try:
                df_kis = KisKR.GetOhlcv(stock_code,"D")
                if df_kis is None:
                    print(f"한투 데이터 조회 실패 ({stock_code}). 토큰을 재발급받습니다...")
                    RefreshTokenIfExpired(GetNowDist())
                    df_kis = KisKR.GetOhlcv(stock_code,"D")
            ...
            if except_riase == True:
                ...
                try:
                    ...
                    print("web.DataReader 마지막 날짜:", df.index[-1])
                    df_last_date = pd.to_datetime(df.index[-1])
                    ...
                    if kis_last_date > df_last_date:
                        ...
                        # 데이터프레임 합치기
                        df = pd.concat([df, new_data])
                        print("web.DataReader 신규 데이터 누락으로 인해 추가 완료!")     
                    else:
                        print("web.DataReader 데이터를 잘 가져옴")
...
#넘겨온 종목 코드 리스트에 해당 종목이 있는지 여부를 체크하는 함수!
...
#스토캐스틱 값 구하기!
def GetStochastic(df, n=14, m=5, t=5):
...
def save_trade_log(strategy_name, code, name, action, price, qty, pnl=None):
    """
    거래 내역을 CSV 파일에 저장합니다. 파일이 없으면 새로 생성하고 헤더를 추가합니다.
    """
    import csv
    import os
    ...
    try:
        # Important: Assumes KIS_Common.py is in the same directory as strategy scripts,
        # and logs directory is a sub-directory.
        script_dir = os.path.dirname(os.path.abspath(__file__))
        logs_dir = os.path.join(script_dir, "logs")
        os.makedirs(logs_dir, exist_ok=True)
        
        trades_csv_path = os.path.join(logs_dir, f"{strategy_name}_trades.csv")
        
        header = ["date", "action", "code", "name", "qty", "price", "pnl"]
        ...
        with open(trades_csv_path, 'a', newline='', encoding='utf-8') as f:
            ...
            pnl_to_write = '' if pnl is None else round(pnl, 2)
            qty_to_write = int(qty) if qty is not None else 0
            
            w.writerow([date_str, action, code, name, qty_to_write, round(price, 4), pnl_to_write])
    except Exception as e:
        print(f"[{strategy_name}] Failed to record trade for {code}: {e}")

def sync_positions_with_wallet(strategy_name):
    """
    지갑 잔고와 포지션 파일을 동기화합니다.
    포지션 파일에는 있지만 지갑에는 없는 종목은 판매 처리합니다.
    """
    try:
        position_file = f"{strategy_name}_positions.json"

        # 실제 보유 종목 가져오기
        holdings = KisKR.GetMyStockList()
        
        holdings_codes = []
        # GetMyStockList()가 dict 리스트를 반환한다고 가정
        if isinstance(holdings, list):
            holdings_codes = [stock['StockCode'] for stock in holdings]

        # 보유 종목이 하나도 없을 경우, 모든 포지션을 판매 처리
        if not holdings_codes:
            msg = "현재 보유 중인 종목이 없습니다. 모든 포지션을 판매 처리합니다."
            print(msg)
        
        # 전략 포지션 파일 읽기
        try:
            with open(position_file, 'r', encoding='utf-8') as f:
                strategy_positions = json.load(f)
        ...
        updated_positions = []
        positions_to_remove_count = 0
        
        # 포지션과 실제 잔고 비교
        for position in strategy_positions:
            stock_code = position['code']
            
            if stock_code not in holdings_codes:
                # 포지션에 있지만 지갑에 없는 경우 -> 판매된 것으로 간주
                msg = f"[{position['name']}({stock_code})] 종목이 지갑에 없어 판매 처리합니다."
                print(msg)

                # 현재가 조회
                current_price = KisKR.GetCurrentPrice(stock_code)
                
                if current_price > 0 and isinstance(current_price, int):
                    # 판매 기록
                    save_trade_log(strategy_name, stock_code, position['name'], 'sell', current_price, position['quantity'])
                    positions_to_remove_count += 1
                else:
                    msg = f"[{position['name']}({stock_code})] 현재가를 조회할 수 없어 판매 처리를 건너뜁니다."
                    print(msg)
                    updated_positions.append(position) # 현재가 조회가 안되면 일단 유지

            else:
                # 포지션에 있고, 지갑에도 있는 경우 -> 유지
                updated_positions.append(position)

        # 변경사항이 있으면 포지션 파일 업데이트
        if positions_to_remove_count > 0:
            try:
                with open(position_file, 'w', encoding='utf-8') as f:
                    json.dump(updated_positions, f, indent=4, ensure_ascii=False)
                msg = f"{position_file} 업데이트 완료. {positions_to_remove_count}개 종목 판매 처리됨."
                print(msg)

            except Exception as e:
                msg = f"{position_file} 파일 저장 중 오류 발생: {e}"
                print(msg)
                
    except Exception as e:
        msg = f"지갑 동기화 중 오류 발생: {e}"
        print(msg)
