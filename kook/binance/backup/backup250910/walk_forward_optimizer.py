...
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'binance_test2'))
from MA_Stoch_backtest import StochBacktest
...
ALL_TRADES_LOG = os.path.join(LOGS_DIR, 'WFO_All_Trades.log')
...
def run_single_backtest_wfo(params_tuple):
    ...
    # 새로운 스토캐스틱 전략인 경우 직접 실행
    if strategy_class == StochBacktest:
        try:
            # StochBacktest 인스턴스 생성
            bot = strategy_class(
            ...
            # 백테스트 실행
            result = bot.run_backtest(df)
            
            return {
                'strategy': 'StochBacktest',
                'params': strategy_params,
                'total_return_pct': result['total_return'],
                'mdd_pct': result['max_drawdown'],
                'num_trades': result['total_trades'],
            }
        except Exception as e:
            print(f"StochBacktest 실행 오류: {e}")
            return {
                'strategy': 'StochBacktest',
                'params': strategy_params,
        ...
        result = run_backtest(
            ...
            fee=0.001,
            leverage=10,
            atr_stop_loss_multiplier=strategy_params.get('atr_multiplier', 2.0),
            trailing_stop_pct=strategy_params.get('trailing_stop_pct'),
            trailing_stop_mode=strategy_params.get('trailing_stop_mode', 'fixed'),
...
def generate_param_grid():
    ...
    # 2. 볼린저 밴드 (ADX 필터) 전략 파라미터 (트레일링스탑 제거)
    bb_adx_params = {
        'window': [20],
        'std_dev': [2],
        'adx_threshold': [20, 25],
    ...
    # 3. MACD (ADX 필터) 전략 파라미터 (트레일링스탑 제거)
    macd_adx_params = {
        'fastperiod': [12],
        'slowperiod': [26],
        'signalperiod': [9],
...
def main(run_id=None):
    ...
    ALL_TRADES_LOG = os.path.join(LOGS_DIR, f'WFO_All_Trades{suffix}.log')
    
    log_realtime("="*80)
    log_realtime(f"🚀 Walk-Forward Optimizer 시작 (Run ID: {run_id or 'default'})")

    # 이전 손실/전체 거래 로그 파일 삭제
    if os.path.exists(LOSING_TRADES_LOG):
        os.remove(LOSING_TRADES_LOG)
    if os.path.exists(ALL_TRADES_LOG):
        os.remove(ALL_TRADES_LOG)
    ...
    while True:
        ...
        oos_result = run_backtest(
            ...
            trailing_stop_mode=best_params.get('trailing_stop_mode', 'fixed'),
            trailing_stop_min=0.01,  # 최소 1%
            trailing_stop_max=0.05,  # 최대 5%
            trades_output_csv=os.path.join(LOGS_DIR, f'Backtest_Trades.csv')  # 거래 내역 저장
        )
        ...
        # 전체 거래 기록 (이모지 포함) 저장
        try:
            with open(ALL_TRADES_LOG, 'a', encoding='utf-8') as f:
                ...
                for trade in oos_result.get('trades', []):
                    ...
                    ep_s = f"{ep:.2f}" if isinstance(ep, (int, float)) else (str(ep) if ep is not None else '-')
                    xp_s = f"{xp:.2f}" if isinstance(xp, (int, float)) else (str(xp) if xp is not None else '-')
                    exit_reason = trade.get('exit_reason', 'unknown')
                    f.write(
                        f"  {emoji} Type: {trade.get('type')}, Entry: {trade.get('entry_time')} @ {ep_s}, Exit: {trade.get('exit_time')} @ {xp_s}, PnL %: {pnl_pct:.2f}%, Reason: {exit_reason}\n"
                    )
        except Exception as e:
            log_realtime(f"  - 전체 거래 로그 기록 중 오류: {e}")
            ...
            try:
                with open(LOSING_TRADES_LOG, 'a', encoding='utf-8') as f:
                    ...
                    for trade in losing_trades:
                        ...
                        exit_reason = trade.get('exit_reason', 'unknown')
                        log_line = (
                            f"  {emoji} Type: {trade.get('type')}, "
                            f"Entry: {trade.get('entry_time')} @ {ep_s}, "
                            f"Exit: {trade.get('exit_time')} @ {xp_s}, "
