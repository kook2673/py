...
def run_single_backtest(params_tuple):
    ...
    result = run_backtest(
        ...
        fee=0.001,
        leverage=10,
        atr_stop_loss_multiplier=strategy_params.get('atr_multiplier', None)
    )
...
def generate_param_grid():
    ...
    bb_grid = [
        (BollingerBandADXStrategy, dict(zip(bb_adx_params.keys(), p)))
        for p in itertools.product(*bb_adx_params.values())
    ]
    ...
    macd_grid = [
        (MacdADXStrategy, dict(zip(macd_params.keys(), p)))
    ...
    stoch_grid = [
        (StochasticStrategy, dict(zip(stoch_params.keys(), p)))
        for p in itertools.product(*stoch_params.values())
    ]


    # --- 생성된 그리드를 합쳐서 반환 ---
    # 여기 주석을 풀거나 추가하여 다른 전략도 테스트 가능
    final_grid = rsi_grid + bb_grid + macd_grid + stoch_grid
