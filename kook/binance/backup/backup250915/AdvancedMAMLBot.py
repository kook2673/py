...
log_file = os.path.join(logs_dir, f"AdvancedMAMLBot_{current_date}.log")
logging.basicConfig(
    level=logging.INFO, 
...
class AdvancedMAMLBot:
    ...
    def auto_tune_parameters(self, df: pd.DataFrame, n_trials: int = 50) -> Dict:
        """파라미터 오토튜닝"""
        logger.info(f"파라미터 오토튜닝 시작... (n_trials={n_trials})")
        
        # 튜닝할 파라미터 범위
        param_ranges = {
            'ma_short': [0, 3, 5, 7, 10, 12],  # 0 추가: 이동평균선 비활성화
            'ma_long': [0, 15, 20, 25, 30, 50],  # 0 추가: 이동평균선 비활성화
            'bb_period': [0, 15, 20, 25],  # 0 추가: 볼린저밴드 비활성화
            'bb_std': [0, 1.5, 2.0, 2.5],  # 0 추가: 볼린저밴드 비활성화
            'rsi_period': [0, 10, 14, 18],  # 0 추가: RSI 비활성화
            'stoch_k': [0, 10, 14, 18],  # 0 추가: 스토캐스틱 비활성화
            'macd_fast': [0, 8, 12, 16],  # 0 추가: MACD 비활성화
            'macd_slow': [0, 20, 26, 32],  # 0 추가: MACD 비활성화
            'macd_signal': [0, 7, 9, 11],  # 0 추가: MACD 비활성화
            'stop_loss_pct': [0.01, 0.015, 0.02],  # 더 작은 스탑로스
            'take_profit_pct': [0.02, 0.025, 0.03],  # 더 작은 익절
            'trailing_stop_pct': [0.01, 0.015, 0.02],  # 더 작은 트레일링 스탑
            'trailing_stop_activation_pct': [0.005, 0.01]  # 더 작은 트레일링 활성화
        }
    ...
    def generate_signal(self, df: pd.DataFrame, features: List[str]) -> Dict:
        ...
        # 이동평균선 신호도 확인 (비활성화된 경우 고려)
        ma_short = df['ma_short'].iloc[-1]
        ma_long = df['ma_long'].iloc[-1]
        current_price = df['close'].iloc[-1]
        
        # 기본 이동평균선 신호 (0일 때는 중립)
        ma_signal = 0
        if ma_short > 0 and ma_long > 0:  # 이동평균선이 활성화된 경우만
            if current_price > ma_short > ma_long:
                ma_signal = 1  # 상승 추세
            elif current_price < ma_short < ma_long:
                ma_signal = -1  # 하락 추세
        # 이동평균선이 비활성화된 경우 ma_signal = 0 (중립)
        
        # 신호 생성 (머신러닝 + 이동평균선 조합)
        # 이동평균선이 비활성화된 경우 머신러닝 신호만 사용
        # prediction: 1=상승, -1=하락, 0=횡보
        if prediction == 1 and (ma_signal >= 0 or (ma_short == 0 and ma_long == 0)) and confidence > 0.5:
            return {'action': 'buy', 'confidence': confidence}
        elif prediction == -1 and (ma_signal <= 0 or (ma_short == 0 and ma_long == 0)) and confidence > 0.5:
    ...
    def close_position(self, position: Dict, price: float, reason: str):
        ...
        # 가격 변동률 계산
        if position['side'] == 'buy':
            price_change_pct = (price - position['price']) / position['price'] * 100
        else:
            price_change_pct = (position['price'] - price) / position['price'] * 100
        
        logger.info(f"포지션 청산: {position['side']} {position['amount']:.4f} @ {price:.2f}, PnL: {pnl:.2f} USDT, 가격변동: {price_change_pct:+.2f}%, 이유: {reason}, 현재 포지션 사이즈: {self.get_current_position_size()}")
    ...
    def save_ml_model(self, model_file: str, scaler_file: str, model_info_file: str):
        """머신러닝 모델 저장 (분리된 파일)"""
        if self.ml_model is None or self.scaler is None:
            logger.warning("저장할 모델 또는 스케일러가 없습니다.")
            return False
        ...
        try:
            ...
            # 모델 정보 저장 (JSON 직렬화를 위해 타입 변환)
            model_info = {
                'model_name': type(self.ml_model).__name__,
                'params': {k: v.item() if hasattr(v, 'item') else v 
                          for k, v in self.params.items()},
                'feature_importance': self.feature_importance,
                'baseline_performance': getattr(self, 'baseline_performance', None),
                'train_date': dt.datetime.now().isoformat()
            }
            
            with open(model_info_file, 'w', encoding='utf-8') as f:
                json.dump(model_info, f, indent=2, ensure_ascii=False)
            
            logger.info(f"ML 모델 저장 완료: {model_file}")
            return True
        except Exception as e:
            logger.error(f"ML 모델 저장 실패: {e}")
            return False
    
    def load_ml_model(self, model_file: str, scaler_file: str, model_info_file: str = None):
        """저장된 머신러닝 모델 로드"""
        try:
            # 모델 로드
            ...
            if model_info_file and os.path.exists(model_info_file):
                ...
                logger.info(f"모델 로드 완료: {model_info.get('model_name', 'Unknown')}")
                logger.info(f"모델 파일: {model_file}")
