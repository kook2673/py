...
try:
    ...
    print(f"⚠️ KIS_Common 모듈을 사용할 수 없습니다: {e}")
    print("📝 CSV 파일 생성 기능이 제한됩니다.")
    Common = None
    KIS_AVAILABLE = False
...
def get_stock_name_from_api(stock_code):
    ...
    try:
        if not KIS_AVAILABLE:
            logging.warning(f"KIS API를 사용할 수 없어 종목명을 가져올 수 없습니다: {stock_code}")
            return stock_code
...
def get_stock_data_and_save_csv(stock_code, stock_name, days=2000):
    """
    개별 종목의 주가 데이터를 가져와서 CSV 파일로 저장하는 함수
    
    Args:
        stock_code (str): 종목 코드
        stock_name (str): 종목명
        days (int): 가져올 데이터 일수 (기본값: 2000일)
    ...
    try:
        if not KIS_AVAILABLE:
            logging.error(f"KIS API를 사용할 수 없어 {stock_name} ({stock_code}) 데이터를 가져올 수 없습니다.")
            return None
            
        logging.info(f"{stock_name} ({stock_code}) 주가 데이터를 가져오는 중...")
        
        # KIS API를 통해 주가 데이터 가져오기
        df = Common.GetOhlcv("KR", stock_code, limit=days)
        
        if df is None or len(df) == 0:
            logging.error(f"{stock_name} ({stock_code}) 주가 데이터를 가져올 수 없습니다.")
            return None
        ...
        if all(col in df.columns for col in required_columns):
            ...
            logging.error(f"{stock_name} ({stock_code}) 필요한 컬럼이 없습니다: {df.columns.tolist()}")
            return None
        ...
        logging.info(f"{stock_name} ({stock_code}) 데이터가 저장되었습니다: {csv_path}")
        logging.info(f"데이터 기간: {df['date'].min()} ~ {df['date'].max()}")
        logging.info(f"총 {len(df)}개 일자 데이터")
        
        return csv_path
        
    except Exception as e:
        logging.error(f"{stock_name} ({stock_code}) 데이터 가져오기 및 저장 중 오류 발생: {e}")
        return None
