            ...
            if enter_side is not None:
                ...
                if entry_same_day:
                    ...
                    if pd.notna(buy_px) and buy_px > 0:
                        ...
                        qty = int((capital * position_fraction * current_multiplier) // buy_px)
                        if qty > 0:
        ...
        if pos is not None and 'pending' in pos:
            ...
            if idx + 1 < len(cur_df.index):
                ...
                if nxt in dates:
                    ...
                    qty = int((capital * position_fraction * multiplier) // buy_px)
                    if qty > 0:
                        cash -= qty * buy_px * (1 + fee_bps/10000.0)
...
if __name__ == '__main__':
    ...
    for market in ['KOSPI', 'KOSDAQ']:
        ...
        # 동적 포지션 사이징 전략 (승리시 0.5배, 패배시 1배)
        # 공격적 동적 포지션 사이징 테스트 (승리 후 공격적, 패배 후 보수적)
        eq_aggressive, tr_aggressive = backtest_etf_strength(
            market=market, start='2018-01-01', end='2025-09-01',
            up_pct=base['up_pct'], up_ctr=base['up_ctr'], up_vol=base['up_vol'],
            down_pct=base['down_pct'], down_ctr=base['down_ctr'], down_vol=base['down_vol'],
            entry_at_open=base['entry_at_open'], entry_same_day=base['entry_same_day'],
            ...
            use_trailing_pct=False, trail_pct=0.05,
            capital=100_000_000, position_fraction=1.0, slippage_bps=0.0,
            dynamic_sizing=True, win_multiplier=1.2, loss_multiplier=0.8,
        )
        
        # 보수적(라이브) 동적 포지션 사이징 테스트 (승리 후 유지, 패배 후 축소)
        eq_conservative, tr_conservative = backtest_etf_strength(
            market=market, start='2018-01-01', end='2025-09-01',
            up_pct=base['up_pct'], up_ctr=base['up_ctr'], up_vol=base['up_vol'],
            down_pct=base['down_pct'], down_ctr=base['down_ctr'], down_vol=base['down_vol'],
            entry_at_open=base['entry_at_open'], entry_same_day=base['entry_same_day'],
        ...
        if not eq.empty:
            ...
            # 동적 포지션 사이징 결과
            if not eq_aggressive.empty:
                trn_dyn, winr_dyn, ntr_dyn = evaluate(eq_aggressive, tr_aggressive)
                logging.info(f"[{market}] 공격적사이징 - Total Return: {trn_dyn:.2f}%  WinRate: {winr_dyn:.2f}%  Trades: {ntr_dyn}")
                eq_aggressive.to_csv(os.path.join(log_dir, f'etf_strength_equity_{market}_aggressive.csv'), header=['equity'])
                tr_aggressive.to_csv(os.path.join(log_dir, f'etf_strength_trades_{market}_aggressive.csv'), index=False)
                
                # 성과 비교
                return_diff = trn_dyn - trn
                logging.info(f"[{market}] 공격적사이징 효과: 수익률 {return_diff:+.2f}%p 차이")

            # 보수적 포지션 사이징 결과
            if not eq_conservative.empty:
                trn_con, winr_con, ntr_con = evaluate(eq_conservative, tr_conservative)
                logging.info(f"[{market}] 보수적사이징 - Total Return: {trn_con:.2f}%  WinRate: {winr_con:.2f}%  Trades: {ntr_con}")
                eq_conservative.to_csv(os.path.join(log_dir, f'etf_strength_equity_{market}_conservative.csv'), header=['equity'])
                tr_conservative.to_csv(os.path.join(log_dir, f'etf_strength_trades_{market}_conservative.csv'), index=False)
                
                # 성과 비교
                return_diff_con = trn_con - trn
                logging.info(f"[{market}] 보수적사이징 효과: 수익률 {return_diff_con:+.2f}%p 차이")
