                ...
                if potential_buy_stocks:
                    ...
                    if selected_stocks_to_buy:
                        ...
                        for stock in selected_stocks_to_buy:
                            ...
                            atr = df.loc[date, 'atr']
                            if atr > 0:
                                # 포지션 사이즈 계산: (총자본 * (종목당 리스크 비율 / 종목수)) / ATR
                                position_size_unit = (strategy_capital * (self.risk_per_trade / self.max_positions)) / atr
            ...
            if is_bull_market:
                ...
                if self.use_pyramiding:
                    for ticker, holding in current_holdings.items():
                        ...
                        # 피라미딩 판단/추가는 기준가만 필요하므로 체결 슬리피지는 적용하지 않음
                        ...
                        if units_count >= self.max_units_per_position:
                            continue
                        # 몇 유닛을 더할 수 있는가 (현재가가 마지막 추가 가격 대비 몇 ATR 올라왔는지)
                        atr_step = self.pyramid_unit_atr * atr
                        if atr_step <= 0:
                            continue
                        steps = int((price - last_add_price) // atr_step)
                        if steps >= 1:
                            add_units = min(steps, self.max_units_per_position - units_count)
                            # 유닛 당 수량 계산
                            position_size_unit = (strategy_capital * (self.risk_per_trade / self.max_positions)) / atr
                            add_qty = int(position_size_unit) * add_units
                            # 목표 수량 상향
...
def run_backtest(universe_dfs, tickers, strategy_class, strategy_params, initial_capital, start_date, end_date):
    ...
    for date in tqdm(backtest_dates, desc=f"Turtle Strategy Backtest ({rebalance_period})"):
        ...
        for ticker, holding in list(portfolio.items()):
            ...
            # 2. 동적 손절매 가격 계산 및 업데이트
            if strategy.use_trailing_stop:
                atr = df.loc[date, 'atr']
                if pd.notna(atr) and atr > 0:
                    trailing_stop_price = holding['highest_price_since_buy'] - (atr * strategy.trailing_stop_atr_multiplier)
                ...
                if target_qty > current_qty:
                    ...
                    if df is not None and date in df.index:
                        ...
                        if cash >= buy_value * (1 + 0.00015):
                            ...
                            stop_loss_price = price - (atr * strategy.stop_loss_atr_multiplier) if pd.notna(atr) and atr > 0 else 0
...
if __name__ == "__main__":
    ...
    # --- 7. 최적화 모드 설정 ---
    OPTIMIZATION_MODE = True # True로 설정 시 최적화 실행, False 시 단일 백테스트 실행
    ...
    if OPTIMIZATION_MODE:
        ...
        param_grid = {
            ...
            'momentum_threshold': [0.65, 0.7, 0.75, 0.8],
            'momentum_sl_threshold': [0.4, 0.5],
            'risk_per_trade': [0.015],
            'max_positions': [20, 30],
            'min_volume_threshold': [500_000_000],
            'use_dual_ma_filter': [False, True],
            'rebalance_period': ['daily', 'weekly'],
            'use_trailing_stop': [False],
            'trailing_stop_atr_multiplier': [3.0],
            'use_pyramiding': [True],
            'pyramid_unit_atr': [0.5, 1.0],
            'max_units_per_position': [3, 4],
            'score_weights_set': [0, 1],
            'slippage_bps': [0.0]
        }
        ...
        results = []
        base_params = {
            'entry_period': 55, 'exit_period': 20, 'atr_period': 20,
            'risk_per_trade': 0.01, 'stop_loss_atr_multiplier': 2.0,
            ...
            'rebalance_period': 'daily', 'use_trailing_stop': False, 'trailing_stop_atr_multiplier': 3.0,
            'use_pyramiding': False, 'pyramid_unit_atr': 0.5, 'max_units_per_position': 4,
        ...
        best_params = {
            ...
            'momentum_sl_threshold': 0.5,
            'min_volume_threshold': 500_000_000,
            'use_market_filter': True,
            'use_dual_ma_filter': False,
            'rebalance_period': 'weekly',  # daily로 변경 - 매일 빈 슬롯 체크
            'use_trailing_stop': False,
            'score_weights': {'ma_slope': 0.2, 'price_ma': 0.25, 'price_change': 0.25, 'volume': 0.15, 'rsi': 0.15},
            'use_pyramiding': True, 'pyramid_unit_atr': 0.5, 'max_units_per_position': 4,
            'slippage_bps': 0.0
        }
