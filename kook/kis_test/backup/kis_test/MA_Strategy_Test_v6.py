...
def load_ma_values_from_json(filename="MA_Strategy_Test_v6.json"):
    """
...
def save_ma_values_to_json(ma_values, filename="MA_Strategy_Test_v6.json"):
    """
...
def get_or_create_ma_values(stock_codes, test_area="KR", filename="MA_Strategy_Test_v6.json"):
    """
...
def setup_logging():
    ...
    log_filename = os.path.join(logs_dir, f"MA_Strategy_Test_v6_{timestamp}.log")
...
#차트 저장 함수
def save_charts(df, stock_code, timestamp, source_dir, strategy_name="MA_Strategy_v6"):
    """
    차트를 저장하는 함수
...
#결과 저장 함수
def save_results(results, timestamp, source_dir, strategy_name="MA_Strategy_v6"):
    """
...
logging.info("MA_Strategy_Test_v6 시작")
...
MAX_INVEST_STOCKS = 40  # 최대 투자 종목 수 20개로 제한
...
for stock_info in InvestStockList:
    ...
    try:
        ...
        #save_charts(df_chart, stock_code, timestamp, source_dir, "MA_Strategy_v6")
    except Exception as e:
    ...
    for investData in NowInvestList:
        ...
        if len(stock_data) == 1:
            ...
            if investData['Investing'] == False and stock_data[str(small_ma)+'ma_before'].values[0] > stock_data[str(big_ma)+'ma_before'].values[0] and stock_data[str(small_ma)+'ma_before2'].values[0] < stock_data[str(small_ma)+'ma_before'].values[0]:
                ...
                if stock_data['Average_Momentum'].values[0] == 1.0 and CurrentInvestCount < MAX_INVEST_STOCKS:
                    IsReblanceDay = True
...
if len(TotalMoneyList) > 0:
    ...
    #차트 저장
    try:
        chart_filename = os.path.join(source_dir, 'charts', f"MA_Strategy_v6_results_{timestamp}.png")
        plt.savefig(chart_filename, dpi=300, bbox_inches='tight')
...
for result in ResultList:
    ...
    try:
        ...
        save_results(results_data, timestamp, source_dir, "MA_Strategy_v6")
