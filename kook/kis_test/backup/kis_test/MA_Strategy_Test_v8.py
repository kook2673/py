'''
1. 균등할당에 코스피100, 코스닥100에 총 200개 종목중에 30개를 비율로 투자
2. Average_Momentum 모먼텀값이 0.4 이하일 경우 0으로 처리
3. 구매시 평균모먼텀이 1인 종목중 기울기가 높은 순으로 구매를 한다.
Average_Momentum이 1인 종목들을 찾음
우선순위 점수 계산 (기울기 관련 요소들):
이동평균선 기울기 (가중치: 1000) - 5일, 20일 이동평균의 기울기
가격 대비 이동평균선 위치 (가중치: 100) - 현재 가격이 20일 이동평균보다 얼마나 위에 있는지
...
def get_top_market_cap_stocks(count=100):
    ...
    try:
        ...
        # 코스피 시가총액 상위 종목 가져오기
        kospi_url = f"https://m.stock.naver.com/api/stocks/NXT/marketValue/KOSPI?page=1&pageSize={count}"
        headers = {
        ...
        # 코스닥 시가총액 상위 종목 가져오기
        kosdaq_url = f"https://m.stock.naver.com/api/stocks/NXT/marketValue/KOSDAQ?page=1&pageSize={count}"
...
def load_ma_values_from_json(filename="MA_Strategy_Test_v8.json"):
    """
...
def save_ma_values_to_json(ma_values, filename="MA_Strategy_Test_v8.json"):
    """
...
def get_or_create_ma_values(stock_codes, test_area="KR", filename="MA_Strategy_Test_v8.json"):
    """
...
def setup_logging():
    ...
    log_filename = os.path.join(logs_dir, f"MA_Strategy_Test_v8_{timestamp}.log")
...
#차트 저장 함수
def save_charts(df, stock_code, timestamp, source_dir, strategy_name="MA_Strategy_v8"):
    """
    차트를 저장하는 함수
...
#결과 저장 함수
def save_results(results, timestamp, source_dir, strategy_name="MA_Strategy_v8"):
    """
...
logging.info("MA_Strategy_Test_v8 시작")
...
MAX_INVEST_STOCKS = 30  # 최대 투자 종목 수 20개로 제한
...
# 투자 비율을 균등하게 설정 (20개 종목에 대해 균등 분배)
invest_rates = [1.0 / MAX_INVEST_STOCKS] * len(stock_codes)  # 모든 종목에 대해 동일한 비율 설정
...
DynamicRate = 1.0 #각 자산별 할당 금액의 60%의 투자 비중은 모멘텀에 의해 정해짐!
...
for stock_info in InvestStockList:
    ...
    # Average_Momentum이 0.2 이하일 때는 0으로 설정
    df.loc[df['Average_Momentum'] <= 0.4, 'Average_Momentum'] = 0.0
    ...
    try:
        ...
        #save_charts(df_chart, stock_code, timestamp, source_dir, "MA_Strategy_v8")
    except Exception as e:
...
for date in combined_df.index.unique():
    ...
    # 현재 날짜에서 Average_Momentum = 1인 종목들을 찾아서 AvailableStockList에 추가 (우선순위 적용)
    ...
    for stock_code in stock_codes:
        ...
        if len(stock_data) == 1:
            if stock_data['Average_Momentum'].values[0] == 1:
                # 우선순위 점수 계산
                priority_score = 0
                
                # 1. 이동평균선 기울기 (추세 강도) - 현재 이동평균과 이전 이동평균의 차이
                try:
                    # 5일 이동평균 기울기 (현재값 - 이전값)
                    ma5_current = stock_data['5ma_before'].values[0]
                    ma5_prev = stock_data['5ma_before2'].values[0] if '5ma_before2' in stock_data.columns else ma5_current
                    ma5_slope = (ma5_current - ma5_prev) / ma5_prev if ma5_prev > 0 else 0
                    
                    # 20일 이동평균 기울기
                    ma20_current = stock_data['20ma_before'].values[0]
                    ma20_prev = stock_data['20ma_before2'].values[0] if '20ma_before2' in stock_data.columns else ma20_current
                    ma20_slope = (ma20_current - ma20_prev) / ma20_prev if ma20_prev > 0 else 0
                    
                    trend_strength = ma5_slope + ma20_slope
                    priority_score += trend_strength * 1000  # 추세 강도 가중치
                except:
                    pass
                
                # 2. 최근 가격 대비 이동평균선 위치 (상승 추세 확인)
                try:
                    current_price = stock_data['close'].values[0]
                    ma20 = stock_data['20ma_before'].values[0]
                    price_vs_ma = (current_price - ma20) / ma20 if ma20 > 0 else 0
                    priority_score += price_vs_ma * 100  # 가격 위치 가중치
                except:
                    pass
                ...
                try:
                    ...
                    priority_score += price_change * 50  # 가격 변화 가중치
                except:
                    pass
                
                candidate_stocks.append((stock_code, priority_score))
    
    # 우선순위 점수가 높은 순서대로 정렬
    candidate_stocks.sort(key=lambda x: x[1], reverse=True)
    AvailableStockList = [stock_code for stock_code, score in candidate_stocks]
    
    # 상위 10개 종목의 우선순위 점수 로깅
    if len(candidate_stocks) > 0:
        logging.info(f"Average_Momentum=1인 종목 {len(candidate_stocks)}개 발견")
        for i, (stock_code, score) in enumerate(candidate_stocks[:10]):
            logging.info(f"우선순위 {i+1}: {stock_code} (점수: {score:.4f})")
    ...
    for investData in NowInvestList:
        ...
        if len(stock_data) == 1:
            ...
            if investData['Investing'] == False and stock_data[str(small_ma)+'ma_before'].values[0] > stock_data[str(big_ma)+'ma_before'].values[0] and stock_data[str(small_ma)+'ma_before2'].values[0] < stock_data[str(small_ma)+'ma_before'].values[0]:
                # Average_Momentum = 1이고 20개 미만일 때만 매수
                if stock_data['Average_Momentum'].values[0] == 1 and CurrentInvestCount < MAX_INVEST_STOCKS:
                    IsReblanceDay = True
                    investData['InvestRate'] = GetDefaultInvestRate(stock_code, StockDataList)
                    logging.info(f"새로운 종목 매수 시도: {stock_code} (Average_Momentum=1)")
        ...
        if investData['IsRebalanceGo'] == True:
            ...
            if len(stock_data) == 1:
                ...
                if investData['RebalanceAmt'] < 0:
                    ...
                    # 매도 후 새로운 종목 구매 시도
                    if CurrentInvestCount < MAX_INVEST_STOCKS and len(AvailableStockList) > 0:
                            ...
                            if not is_already_investing:
                                ...
                                for new_invest in NowInvestList:
                                    if new_invest['stock_code'] == available_stock:
                                        ...
                                        logging.info(f"매도 후 새로운 종목 구매 시도: {available_stock}")
                                        break
                                break
                ...
                if investData['RebalanceAmt'] > 0:
                    ...
                    if BuyAmt > 0 and RemainInvestMoney > NowClosePrice:
                        ...
                        if investData['stock_code'] in AvailableStockList:
                            logging.info(f"매도 후 새로운 종목 매수 완료: {investData['stock_code']}")
...
if len(TotalMoneyList) > 0:
    ...
    #차트 저장
    try:
        chart_filename = os.path.join(source_dir, 'charts', f"MA_Strategy_v8_results_{timestamp}.png")
        plt.savefig(chart_filename, dpi=300, bbox_inches='tight')
...
for result in ResultList:
    ...
    try:
        ...
        save_results(results_data, timestamp, source_dir, "MA_Strategy_v8")
