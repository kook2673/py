"""
그리고 수수료 걱정하시는 분들은 전략을 어떻게 짜셨는지 모르겠는데...

제가 어제 180달러로 레버리지 30배 써서 5분봉 단타 했는데..

펀딩비와 수수료 내역입니다.

아...저거는 야료가 좀 있어요.저는 손절 없이 수익만 5% 되면 그때부터 청산 트리거 발동.
따라서 손실 없이 수익만 나오기 때문에 미청산 손실은 않 나옵니다.

어제 40달러 정도 수익 났었고요...

일단 추세 추종이라 신호 나오면 진입. 단 심볼에 따라 다르지만 진입 심볼이 일정비율 손실이 되면 저는 반대 포지션 잡아요.

크로스 모드이고 전체자산을 30으로 나누고, 거기다 레버리지를 곱하고, 롱과 숏을 50대50 으로 처음부터 설정해 둬서 네버 강제 청산 엔딩 없어요

처음에 저도 격리로 했는데 변동성 큰 종목은 오히려 격리 강제 청산이 빨리 되서 손실만 더 키우는 꼴이라

예늘 들어 300달러를 30종목으로 자산분배 하면 심볼당 10달러.
거기다 롱숏 50%씩 분배하면 심볼당 방향별 최대 가능금액은 5달러
레버리지 20배면 심볼당 100달러가 방향별 진입 가능 금액.
추세 맞으면 양방 없이 계속 5% 수익부터 트리거되서 분할 청산

진입 후 일정손실 발생하면 반대포지션 진입

더 추가할 로직이 오늘 아침에 생각나서...

마진 발란스가 오전 9시 대비 20% 증가하면 수익, 손실 상관 없이 보유 포지션을 일정 비율 청산.

제 로직은 수익금보다 마진 발란스의 증감이 중요해서요

맞아요.
미청산 수익, 손실은 별 의미가 없어서, 수익이든 손실이든 지금 당장 다 청산하고 현금화 할 수 있는 자산이 제일 중요.

마진 발란스가 지금 당장 올청산하고 현금화할 수 있는 자산이예요.
이미 미청산 수익이나 손실 다 반영된...
"""
