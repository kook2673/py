...
def convert_5m_to_15m():
    """5분 데이터를 15분 데이터로 변환"""
    
    # 5분 데이터 디렉토리
    input_dir = "data/BTCUSDT/5m"
    output_dir = "data/BTCUSDT/15m"
    ...
    for year in years:
        ...
        output_file = f"{output_dir}/BTCUSDT_15m_{year}.csv"
        ...
        if os.path.exists(input_file):
            ...
            print(f"  완료: {len(df_15m)}개 캔들 -> {output_file}")
            print(f"  기간: {df_15m['timestamp'].min()} ~ {df_15m['timestamp'].max()}")
        else:
