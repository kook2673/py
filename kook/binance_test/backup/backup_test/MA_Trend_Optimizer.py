...
def load_data_for_optimization(year: int, month: int, data_dir: str, lookback_candles: int = 500, timeframe: str = '4h') -> pd.DataFrame:
    ...
    for check_year in range(2023, year + 1):
        ...
        if os.path.exists(year_file):
            ...
            # 1시간봉 데이터는 'timestamp' 컬럼 사용
            if 'timestamp' in year_df.columns:
                year_df['datetime'] = pd.to_datetime(year_df['timestamp'])
            else:
...
def optimize_ma_parameters(df: pd.DataFrame, ma1_range: Tuple[int, int] = (3, 21), 
                           ma2_range: Tuple[int, int] = (22, 200), 
                           strategy_type: str = 'LONG') -> Optional[Dict]:
    """MA 파라미터 최적화 (스마트 샘플링으로 속도 개선)"""
    print(f"🔍 MA 파라미터 최적화 중... ({strategy_type})")
    print(f"   MA1 범위: {ma1_range[0]} ~ {ma1_range[1]} (스마트 샘플링)")
    print(f"   MA2 범위: {ma2_range[0]} ~ {ma2_range[1]} (스마트 샘플링)")
    
    best_score = -999999
    best_result = None
    
    # 전체 범위 검색: 3~21, 20~200 전체 값 사용
    ma1_values = list(range(ma1_range[0], ma1_range[1] + 1))
    ma2_values = list(range(ma2_range[0], ma2_range[1] + 1))
    
    total_combinations = len(ma1_values) * len(ma2_values)
    print(f"📊 총 {total_combinations}개 MA 조합 테스트 중... (전체 범위 검색)")
    print(f"   MA1 범위: {ma1_range[0]}~{ma1_range[1]} (전체 {len(ma1_values)}개 값)")
    print(f"   MA2 범위: {ma2_range[0]}~{ma2_range[1]} (전체 {len(ma2_values)}개 값)")
    ...
    for ma1 in ma1_values:
        for ma2 in ma2_values:
            ...
            # 기본 추세 파라미터로 백테스트 (숏 전략은 더 완화된 조건)
            if strategy_type == 'SHORT':
                basic_trend_params = {
                    'trend_continuity_min': 2,  # 더 낮은 연속성 요구
                    'rsi_oversold': 70,  # 숏 전략용 RSI 과매수
                    'rsi_overbought': 30,  # 숏 전략용 RSI 과매도
                    'momentum_period': 3,  # 더 짧은 모멘텀
                    'bb_threshold': 0.3,  # 더 낮은 볼린저 밴드 임계값
                    'volume_multiplier': 1.0  # 볼륨 요구사항 완화
                }
            else:
                basic_trend_params = {
                    'trend_continuity_min': 3,
                    'rsi_oversold': 30,
                    'rsi_overbought': 80,
                    'momentum_period': 5,
                    'bb_threshold': 0.5,
            ...
            result = backtest_strategy(df.copy(), ma1, ma2, basic_trend_params, strategy_type=strategy_type)
...
def main():
    ...
    for year in range(start_year, end_year + 1):
        for month in range(start_month, end_month + 1):
            ...
            # month_key가 없으면 새로 생성
            if month_key not in existing_params:
                existing_params[month_key] = {
                    "long_strategy": {},
                    "short_strategy": {},
                    "optimization_info": {}
                }
                print(f"🆕 {year}년 {month}월: 새로운 월 추가. 전체 최적화를 진행합니다.")
            else:
                # 롱/숏 전략이 모두 있는지 확인
            ...
            # 1단계: MA 파라미터 최적화 (롱 전략)
            if "long_strategy" not in existing_params[month_key] or not existing_params[month_key]["long_strategy"]:
                print(f"\n🔧 1단계: MA 파라미터 최적화 (롱 전략)")
                ma_result_long = optimize_ma_parameters(combined_df.copy(), ma1_range=(3, 21), ma2_range=(20, 200), strategy_type='LONG')
                
                if not ma_result_long:
                    print(f"❌ 롱 전략 MA 최적화 실패. {year}년 {month}월 건너뜁니다.")
                    continue
            else:
            ...
            # 2단계: 추세 파라미터 최적화 (롱 전략)
            if "long_strategy" not in existing_params[month_key] or "trend_params" not in existing_params[month_key]["long_strategy"] or not existing_params[month_key]["long_strategy"]["trend_params"]:
                print(f"\n🔧 2단계: 추세 파라미터 최적화 (롱 전략)")
                trend_result_long = optimize_trend_parameters(combined_df.copy(), ma_result_long['ma1'], ma_result_long['ma2'])
                
                if not trend_result_long:
                    print(f"❌ 롱 전략 추세 파라미터 최적화 실패. 기본값 사용.")
                    trend_result_long = {
                        'trend_params': {
                            'trend_continuity_min': 3,
                            'rsi_oversold': 30,
                ...
                print(f"⏭️  롱 전략 추세 파라미터는 이미 최적화 완료. 기존 값 사용.")
                trend_result_long = {
                    'trend_params': existing_params[month_key]['long_strategy']['trend_params']
                }
            
            # 4단계: 숏 전략 최적화
            if "short_strategy" not in existing_params[month_key] or not existing_params[month_key]["short_strategy"]:
                print(f"\n🔧 4단계: 숏 전략 최적화")
                ma_result_short = optimize_ma_parameters(combined_df.copy(), ma1_range=(3, 21), ma2_range=(20, 200), strategy_type='SHORT')
                
                if not ma_result_short:
                    print(f"⚠️  숏 전략 MA 최적화 실패. 롱 전략만 저장합니다.")
                    ma_result_short = ma_result_long.copy()
                    trend_result_short = trend_result_long.copy()
                else:
                    # 5단계: 숏 전략 추세 파라미터 최적화
                    print(f"\n🔧 5단계: 숏 전략 추세 파라미터 최적화")
                    trend_result_short = optimize_trend_parameters(combined_df.copy(), ma_result_short['ma1'], ma_result_short['ma2'])
                    
                    if not trend_result_short:
                        print(f"⚠️  숏 전략 추세 파라미터 최적화 실패. 기본값 사용.")
                        trend_result_short = {
                            'trend_params': {
                                'trend_continuity_min': 3,
                                'rsi_oversold': 70,
                ...
                if "trend_params" in existing_params[month_key]["short_strategy"] and existing_params[month_key]["short_strategy"]["trend_params"]:
                    print(f"⏭️  숏 전략 추세 파라미터도 이미 최적화 완료. 기존 값 사용.")
                    trend_result_short = {
                        'trend_params': existing_params[month_key]['short_strategy']['trend_params']
                    }
                else:
                    print(f"🔧 숏 전략 추세 파라미터만 추가 최적화합니다.")
                    trend_result_short = optimize_trend_parameters(combined_df.copy(), ma_result_short['ma1'], ma_result_short['ma2'])
