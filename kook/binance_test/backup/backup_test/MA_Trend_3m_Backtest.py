#-*-coding:utf-8 -*-
'''
바이낸스 선물거래 MA + 추세 하이브리드 전략 백테스트 (3분봉, JSON MA 값 사용)
2024년 1월부터 현재까지 월간별로 JSON에서 MA 값을 읽어서 백테스트 실행
...
def backtest_hybrid_strategy(df, ma1, ma2, initial_capital=10000, leverage=3, fee=0.001, use_trend=False, trend_params=None, strategy_type='LONG'):
    """MA + 추세 하이브리드 전략 백테스트 (롱/숏 지원)"""
    # MA 이동평균선 계산
    ...
    peak_price = 0  # 롱의 경우 고점, 숏의 경우 저점
    ...
    for i in range(2, len(df)):
        ...
        if use_trend and i >= 20:
            ...
            if (momentum_5 > 0 and momentum_10 > 0 and  # 모멘텀 양수
                ...
                trend_signal_long = 1  # 상승 추세
            
            # 숏 추세 신호 생성 (반대 조건)
            if (momentum_5 < 0 and momentum_10 < 0 and  # 모멘텀 음수
                trend_continuity <= -trend_continuity_min and  # JSON에서 가져온 연속 하락 기준
                bb_position < 0.7 and                     # 볼린저 밴드 0.7 이하
                rsi < rsi_overbought and                  # JSON에서 가져온 RSI 과매수 기준
                rsi > rsi_oversold):                      # JSON에서 가져온 RSI 과매도 기준
                trend_signal_short = 1  # 하락 추세
        ...
        if position == 0:
            ...
            # 롱 진입 신호 (MA + 추세 조합)
            ma_signal_long = (close_prev >= ma1_prev and ma1_prev2 <= ma1_prev and 
            ...
            # 롱 진입 조건
            if use_trend:
                long_entry_condition = ((ma_signal_long and trend_signal_long == 1) or 
                                      (trend_signal_long == 1 and momentum_5 > 0 and momentum_10 > 0)) and (i < 20 or volume_filter)
            else:
                long_entry_condition = ma_signal_long and (i < 20 or volume_filter)
            
            # 숏 진입 조건
            if use_trend:
                short_entry_condition = ((ma_signal_short and trend_signal_short == 1) or 
                                       (trend_signal_short == 1 and momentum_5 < 0 and momentum_10 < 0)) and (i < 20 or volume_filter)
            else:
                short_entry_condition = ma_signal_short and (i < 20 or volume_filter)
            ...
            if long_entry_condition:
                ...
                entry_time = current_time
                position_size = (total_capital * leverage) / current_price
                peak_price = current_price  # 트레일링스탑용 고점 초기화
                
                # 전략 타입 결정
                if use_trend and trend_signal_long == 1:
                    strategy_type = 'TREND_LONG'
                else:
                    strategy_type = 'MA_LONG'
            ...
            # 숏 진입
                ...
                entry_time = current_time
                position_size = (total_capital * leverage) / current_price
                peak_price = current_price  # 트레일링스탑용 저점 초기화
                
                # 전략 타입 결정
                if use_trend and trend_signal_short == 1:
                    strategy_type = 'TREND_SHORT'
                else:
                    strategy_type = 'MA_SHORT'
        ...
        # 포지션 관리 (롱/숏)
            ...
            if position == 1:  # 롱 포지션
                ...
                if profit_pct >= 10 and not trailing_stop_active:
                    trailing_stop_active = True
                    trailing_stop_price = peak_price * 0.97  # 고점 대비 3% 하락 시 청산
                
                # 트레일링스탑 업데이트
                if trailing_stop_active:
                    trailing_stop_price = max(trailing_stop_price, peak_price * 0.97)
                ...
                # 추세 전환
                elif use_trend and i >= 20:
                    if (momentum_5 < 0 or momentum_10 < 0) and trend_continuity <= 0:
                        should_close = True
                ...
                if should_close:
                    ...
                    equity_curve.append({
                        ...
                        'price': current_price,
                        'pnl_display': f"롱청산: {entry_price:.0f} | 청산: {current_price:.0f} | 실현: {pnl*100:.2f}% | 수익: {net_pnl:.0f} | {strategy_type} | {close_reason}"
                    })
                ...
                if profit_pct >= 10 and not trailing_stop_active:
                    trailing_stop_active = True
                    trailing_stop_price = peak_price * 1.03  # 저점 대비 3% 상승 시 청산
                
                # 트레일링스탑 업데이트
                if trailing_stop_active:
                    trailing_stop_price = min(trailing_stop_price, peak_price * 1.03)
                ...
                if should_close:
                    ...
                    equity_curve.append({
                        ...
                        'equity': total_capital,
                        'price': current_price,
                        'pnl_display': f"숏청산: {entry_price:.0f} | 청산: {current_price:.0f} | 실현: {pnl*100:.2f}% | 수익: {net_pnl:.0f} | {strategy_type} | {close_reason}"
                    })
        ...
        if position != 0 and current_time == entry_time:
            position_type = "롱" if position == 1 else "숏"
            equity_curve.append({
                'time': current_time,
                'equity': total_capital,
                'price': current_price,
                'pnl_display': f"{position_type}진입: {entry_price:.0f} | 미실현: 0.00% | {strategy_type}"
            })
...
def plot_backtest_results(df, backtest_result, ticker):
    ...
    if not df.empty:
        ...
        if not chart_data.empty:
            ...
            ax1.plot(chart_data.index, chart_data['close'], 
                    label='BTC/USDT 3M', linewidth=1.5, color='lightblue', alpha=0.9)
    ...
    ax1.set_title(f'{ticker} - 3분봉 가격 차트 및 매매 신호', fontsize=14)
    ax1.legend()
...
def load_optimized_parameters(json_file_path):
    """MA_Trend_Optimizer_3m.json에서 최적화된 파라미터 로드"""
    try:
...
def main():
    """메인 함수"""
    print("바이낸스 선물거래 MA + 추세 하이브리드 전략 백테스트 시작! (3분봉)")
    print("MA_Trend_Optimizer_3m.json에서 최적화된 파라미터를 읽어와서 2024년부터 현재까지 백테스트를 실행합니다.")
    print("전략: 롱 + 숏 하이브리드 (자본 50:50 분할, 트레일링스탑 + 손절)")
    
    # 롱 + 숏 하이브리드 전략으로 고정
    selected_strategy = 'LONG_SHORT_HYBRID'
    print(f"\n✅ 사용 전략: {selected_strategy}")
    
    # MA_Trend_Optimizer_3m.json 파일에서 최적화된 파라미터 로드
    script_dir = os.path.dirname(os.path.abspath(__file__))
    json_file_path = os.path.join(script_dir, "MA_Trend_Optimizer_3m.json")
    
    if not os.path.exists(json_file_path):
        print(f"❌ MA_Trend_Optimizer_3m.json 파일을 찾을 수 없습니다.")
        print(f"📁 파일 경로: {json_file_path}")
        return
    ...
    # 2024년부터 현재까지의 CSV 파일들 로드 (3분봉)
    data_dir = os.path.join(script_dir, 'data', 'BTCUSDT', '3m')
    
    # 2024년부터 2025년까지의 CSV 파일들 찾기 (3분봉)
    csv_files = []
    for year in [2024, 2025]:
        csv_pattern = f'BTCUSDT_3m_{year}.csv'
    ...
    # 모든 데이터 합치기
    df_3m = pd.concat(all_data, ignore_index=False)
    df_3m = df_3m.sort_index()  # 시간순 정렬
    
    print(f"✅ 전체 데이터 로드 완료: {len(df_3m)}개 캔들")
    print(f"기간: {df_3m.index[0]} ~ {df_3m.index[-1]}")
    
    # 월간별 백테스트 결과 저장
    monthly_backtest_results = []
    
    # 각 월별로 백테스트 진행 (롱 + 숏 전략 각각 실행)
    ...
    for month_key, params in monthly_parameters.items():
        ...
        month_df = df_3m[(df_3m.index >= month_start_date) & (df_3m.index <= month_end_date)]
        ...
        # 롱 전략 파라미터
        long_ma1 = params['long_strategy']['ma1']
        long_ma2 = params['long_strategy']['ma2']
        long_trend_params = params['long_strategy'].get('trend_params', {})
        
        # 숏 전략 파라미터
        short_ma1 = params['short_strategy']['ma1']
        short_ma2 = params['short_strategy']['ma2']
        short_trend_params = params['short_strategy'].get('trend_params', {})
        
        print(f"\n{'='*60}")
        print(f"진행률: {len(monthly_backtest_results)+1}/{len(monthly_parameters)} 월")
        print(f"기간: {year}년 {month:02d}월")
        print(f"롱 전략: MA1={long_ma1}, MA2={long_ma2}")
        print(f"숏 전략: MA1={short_ma1}, MA2={short_ma2}")
        print(f"현재 자본: {current_capital:.2f} USDT")
        print(f"전략: {selected_strategy} (롱 + 숏 하이브리드)")
        ...
        # 롱 전략 실행
        print(f"\n--- 롱 전략 실행 (자본: {long_capital:.2f} USDT) ---")
        long_result = backtest_hybrid_strategy(month_df.copy(), long_ma1, long_ma2, long_capital, leverage, use_trend=True, trend_params=long_trend_params)
        
        # 숏 전략 실행
        print(f"\n--- 숏 전략 실행 (자본: {short_capital:.2f} USDT) ---")
        short_result = backtest_hybrid_strategy(month_df.copy(), short_ma1, short_ma2, short_capital, leverage, use_trend=True, trend_params=short_trend_params)
        
        if long_result and short_result:
            # 전체 결과 계산
            total_final_equity = long_result['final_equity'] + short_result['final_equity']
            total_return = (total_final_equity - current_capital) / current_capital * 100
            
            # 전체 거래 통계
            total_trades = long_result['trade_count'] + short_result['trade_count']
            total_win_trades = long_result['win_trades'] + short_result['win_trades']
            
            # 전체 MDD 계산 (더 복잡한 로직 필요)
            total_mdd = max(long_result['mdd'], short_result['mdd'])
            ...
            monthly_result = {
                ...
                'long_strategy': {
                    ...
                    'equity': long_result['final_equity'],
                    'mdd': long_result['mdd'],
                    'trade_count': long_result['trade_count'],
                    'win_trades': long_result['win_trades']
                },
                'short_strategy': {
                    'ma1': short_ma1,
                    'ma2': short_ma2,
                    'return': short_result['total_return'],
                    'equity': short_result['final_equity'],
                    'mdd': short_result['mdd'],
                    'trade_count': short_result['trade_count'],
                    'win_trades': short_result['win_trades']
                },
                'total_return': total_return,
                ...
                'equity_curve': long_result['equity_curve'] + short_result['equity_curve'],
                'trades': long_result['trades'] + short_result['trades'],
                'strategy_mode': 'LONG_SHORT_HYBRID'
            }
            ...
            # 월간 결과 출력
            print(f"\n=== 월간 백테스트 결과 ===")
            print(f"롱 전략: 수익률 {long_result['total_return']:.2f}%, MDD {long_result['mdd']:.2f}%, 거래 {long_result['trade_count']}회")
            print(f"숏 전략: 수익률 {short_result['total_return']:.2f}%, MDD {short_result['mdd']:.2f}%, 거래 {short_result['trade_count']}회")
            print(f"전체: 수익률 {total_return:.2f}%, MDD {total_mdd:.2f}%, 거래 {total_trades}회")
            
            if total_trades > 0:
                win_rate = total_win_trades / total_trades * 100
                print(f"전체 승률: {win_rate:.1f}%")
            
            # 다음 달 초기 자본으로 업데이트
            current_capital = total_final_equity
    ...
    if monthly_backtest_results:
        ...
        print(f"\n=== 전체 성과 요약 ===")
        print(f"사용 전략: {selected_strategy} (롱 + 숏 하이브리드)")
        ...
        for i, result in enumerate(monthly_backtest_results):
            print(f"{result['month_start'].strftime('%Y-%m')}: "
                  f"롱(MA1={result['long_strategy']['ma1']}, MA2={result['long_strategy']['ma2']}, 수익률={result['long_strategy']['return']:.2f}%), "
                  f"숏(MA1={result['short_strategy']['ma1']}, MA2={result['short_strategy']['ma2']}, 수익률={result['short_strategy']['return']:.2f}%), "
                  f"전체={result['total_return']:.2f}%")
    ...
    final_results = {
        ...
        'strategy_description': '롱 + 숏 하이브리드 전략 (자본 50:50 분할, 트레일링스탑 + 손절, JSON 파라미터 사용)',
        'initial_capital': initial_capital,
        'leverage': leverage,
        'backtest_period': {
            ...
            if result and 'trades' in result:
                for trade in result['trades']:
                    ...
                    if trade.get('position_type') == 'LONG':
                        ...
                        if trade['pnl'] > 0:
                            long_win_count += 1
                    elif trade.get('position_type') == 'SHORT':
                        short_trades_count += 1
                        if trade['pnl'] > 0:
                            short_win_count += 1
        
        long_win_rate = (long_win_count / long_trades_count * 100) if long_trades_count > 0 else 0
        short_win_rate = (short_win_count / short_trades_count * 100) if short_trades_count > 0 else 0
    ...
    filename = f'LongShort_3m_Backtest_Result_{ticker.replace("/", "_")}_{selected_strategy}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    ...
    if monthly_backtest_results:
            ...
            if all_equity_curve:
                ...
                # 전체 기간 그래프 생성
                fig = plot_backtest_results(df_3m, total_backtest_result, ticker)
                if fig:
