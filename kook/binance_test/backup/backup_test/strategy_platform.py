...
def calculate_rsi(series: pd.Series, period: int = 14) -> pd.Series:
    """RSI(Relative Strength Index) 지표 계산"""
    delta = series.diff(1)
...
class RsiMeanReversion(Strategy):
    ...
    def _add_indicators(self):
        ...
        self.df['rsi'] = calculate_rsi(self.df['close'], period=self.params.get('rsi_window', 14))
...
class MachineLearningStrategy(Strategy):
    ...
    def __init__(self, df: pd.DataFrame, params: dict, **kwargs):
        self.model = kwargs.get('model', None) # 외부에서 학습된 모델을 받을 수 있도록 수정
        super().__init__(df, params)
        self.features = ['rsi', 'bb_upper', 'bb_lower', 'bb_width', 'bb_position', 'macd', 'macdsignal', 'momentum']

    def _add_indicators_on_df(self, df: pd.DataFrame):
        """주어진 데이터프레임에 지표를 추가하는 헬퍼 함수"""
        df['rsi'] = talib.RSI(df['close'], timeperiod=14)
        upper, middle, lower = talib.BBANDS(df['close'], timeperiod=20)
        df['bb_upper'] = upper
        df['bb_lower'] = lower
        # 0으로 나누기 방지
        middle[middle == 0] = 1e-6 
        lower[upper == lower] = lower[upper == lower] - 1e-6
    ...
    def _create_labels_on_df(self, df: pd.DataFrame):
        """주어진 데이터프레임에 정답(Label)을 추가하는 헬퍼 함수"""
        future_period = self.params.get('future_period', 5)
        fee = self.params.get('fee', 0.001)
        df['future_price'] = df['close'].shift(-future_period)
        df['price_change_pct'] = (df['future_price'] - df['close']) / df['close']
        df['target'] = 0
        df.loc[df['price_change_pct'] > fee, 'target'] = 1

    def _add_indicators(self):
        """모델 학습에 사용할 여러 기술적 지표(특징)들을 계산 (라이브 환경과 동일하게 수정)"""
        self._add_indicators_on_df(self.df)

    def _create_labels(self):
        """미래 가격 방향에 대한 정답(Label) 데이터 생성 (라이브 환경과 동일하게 수정)"""
        self._create_labels_on_df(self.df)

    def train(self, train_df: pd.DataFrame):
        """
        주어진 데이터프레임 전체를 사용하여 모델을 훈련하고 반환합니다.
        WFO의 Out-of-Sample 테스트 전에 호출하기 위한 메서드입니다.
        """
        df = train_df.copy()
        
        self._add_indicators_on_df(df)
        self._create_labels_on_df(df)
        
        df.dropna(inplace=True)
        if len(df) < 100:
            print("경고: 모델 훈련을 위한 데이터가 부족합니다.")
            return None
            
        X_train = df[self.features]
        y_train = df['target']
        
        print(f"최종 모델 훈련 중... (데이터: {len(X_train)}개)")
    ...
    def generate_signals(self) -> pd.Series:
        """모델을 훈련하거나, 제공된 모델로 예측하여 매매 신호를 생성"""
        
        signals = pd.Series(index=self.df.index, data='hold')
        # self.df는 __init__에서 이미 지표가 추가되었음
        df_predict_ready = self.df.dropna(subset=self.features)
        
        if df_predict_ready.empty:
            print("경고: 예측할 데이터가 없습니다 (지표 계산 후 NaN).")
            return signals

        X_predict = df_predict_ready[self.features]
        predict_indices = df_predict_ready.index
        
        model_to_use = self.model

        # 외부 모델이 없으면 (in-sample 최적화 시), 내부에서 7:3 분할 후 임시 모델 훈련
        if model_to_use is None:
            self._create_labels()
            df_labeled = self.df.dropna()
            ...
            X_predict = X_test
            predict_indices = X_test.index
            model_to_use = temp_model

        # 예측 수행
        predictions = model_to_use.predict(X_predict)
        
        # 예측 결과를 신호로 변환
        signals.loc[predict_indices[predictions == 1]] = 'buy'
        signals.loc[predict_indices[predictions == 0]] = 'sell'
...
def run_backtest(
    ...
    Args:
        ...
        stop_loss_pct (float, optional): 손절매 비율 (예: 0.02는 2%)
    ...
    for i in range(len(df)):
        ...
        # --- 0. 손절/익절 조건 확인 (포지션 보유 시 최우선으로) ---
        if position_type is not None:
            exit_reason = None
            pnl_pct_current = 0 # 레버리지 미적용 순수 가격 변동률

            if position_type == 'long':
                pnl_pct_current = (current_price / entry_price) - 1
                if stop_loss_pct is not None and pnl_pct_current < -stop_loss_pct:
                    exit_reason = 'stop_loss'
                elif take_profit_pct is not None and pnl_pct_current > take_profit_pct:
                    exit_reason = 'take_profit'

            elif position_type == 'short':
                pnl_pct_current = (entry_price / current_price) - 1
                if stop_loss_pct is not None and pnl_pct_current < -stop_loss_pct:
                    exit_reason = 'stop_loss'
                elif take_profit_pct is not None and pnl_pct_current > take_profit_pct:
                    exit_reason = 'take_profit'
            ...
            if exit_reason:
                if position_type == 'long':
                    ...
                    pnl_pct_final = ((current_price / entry_price) - 1) * 100
                else: # short
                    pnl = (entry_price - current_price) * position_size * (1 - (fee * 2))
                    pnl_pct_final = ((entry_price / current_price) - 1) * 100
                ...
                position_type = None
                signal = 'hold' # 강제 종료 후에는 이번 타임스텝에서 재진입 방지
...
if __name__ == '__main__':
    ...
    params_to_test_4 = {
        'future_period': 5,
        'n_estimators': 100,
        'fee': 0.001 # 머신러닝 전략에 수수료 파라미터 추가
    }
