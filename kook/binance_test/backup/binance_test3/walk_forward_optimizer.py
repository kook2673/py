...
def run_single_backtest_wfo(params_tuple):
    ...
    result = run_backtest(
        ...
        fee=0.002,  # 0.2% 수수료 (현실적)
        leverage=5,  # 5배 레버리지 (위험 감소)
...
def generate_param_grid():
    ...
    # 4. 스토캐스틱 전략 파라미터 (보수적 설정)
    stoch_params = {
        'fastk_period': [21, 28],           # 더 긴 기간 (노이즈 감소)
        'slowk_period': [5],                # 1개로 축소
        'slowd_period': [5],                # 1개로 축소
        'oversold': [10, 15],               # 더 극단적 (2개로 축소)
        'overbought': [85, 90],             # 더 극단적 (2개로 축소)
        'atr_multiplier': [2.5, 3.0]        # 더 큰 스탑로스 (2개로 축소)
    }
