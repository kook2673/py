...
for Target_Coin_Ticker in Coin_Ticker_List:
    ...
    # 스토캐스틱 계산 함수
    def calculate_stochastic(df, k_period=14, d_period=3, offset=0):
        """스토캐스틱 %K, %D 계산"""
        if len(df) < k_period + offset:
            return 50.0, 50.0  # 기본값
        
        # 현재 인덱스
        current_idx = len(df) - 1 - offset
        
        # %K 계산
        low_min = df['low'].iloc[current_idx-k_period+1:current_idx+1].min()
        high_max = df['high'].iloc[current_idx-k_period+1:current_idx+1].max()
    ...
    # 스토캐스틱 값 계산
    stoch_k, stoch_d = calculate_stochastic(df, 14, 3, 2)
    prev_stoch_k, _ = calculate_stochastic(df, 14, 3, 3)
    ...
    if len(dic['item']) == 0:
        if abs(amt_s) > 0:#값이 있으면 리스트에 추가를 해준다.
            dic['no'] += 1
            dic["item"].append({'no':dic['no'], 'type':'M','enter_price':coin_price,'amt':round(-(abs(amt_s)), 3)})
            msg = "없는 숏포지션 추가\n현재 가격으로 추가합니다.\n실제 구매하신 가격하고 다를수 있습니다."
            viewlist(msg)
        if abs(amt_l) > 0:#값이 다르면 리스트에 추가를 해준다.
            dic['no'] += 1
            dic["item"].append({'no':dic['no'], 'type':'M','enter_price':coin_price,'amt':round(amt_l, 3)})
            msg = "없는 롱포지션 추가\n현재 가격으로 추가합니다.\n실제 구매하신 가격하고 다를수 있습니다."
            viewlist(msg)

        # MA + 스토캐스틱 신호 체크
        # 롱 진입: MA 상승 + 스토캐스틱 과매도에서 상승 전환
        long_signal = (ma5[0] > ma20 and ma5[2] > ma5[1] and ma5[1] < ma5[0] and 
                      stoch_k < 20 and stoch_k > prev_stoch_k)
        
        # 숏 진입: MA 하락 + 스토캐스틱 과매수에서 하락 전환
        short_signal = (ma5[0] < ma20 and ma5[2] < ma5[1] and ma5[1] > ma5[0] and 
                       stoch_k > 80 and stoch_k < prev_stoch_k)
        ...
        if short_signal:
            ...
            dic['no'] += 1
            dic["item"].append({'no':dic['no'], 'type':'N','enter_price':buy_price,'amt':round(-first_amount, 3)})
            msg = "\n"+Target_Coin_Symbol+" 숏 포지션 구매 (MA+Stoch)"
            viewlist(msg)
        ...
        if long_signal:
            ...
            dic['no'] += 1
            dic["item"].append({'no':dic['no'], 'type':'N','enter_price':buy_price,'amt':round(first_amount, 3)})
            msg = "\n"+Target_Coin_Symbol+" 롱 포지션 구매 (MA+Stoch)"
            viewlist(msg)
    ...
    #포지션이 있을경우
        ...
        if abs(amt_s) > amt_s2 and (abs(amt_s)-amt_s2) >= first_amount:#값이 다르면 리스트에 추가를 해준다.
            dic['no'] += 1
            dic["item"].append({'no':dic['no'],'type':'M','enter_price':coin_price,'amt':round(-(abs(amt_s)-amt_s2), 3)})
            msg = "없는 숏포지션 추가\n현재 가격으로 추가합니다.\n실제 구매하신 가격하고 다를수 있습니다."
            viewlist(msg)
        if abs(amt_l) > amt_l2 and (abs(amt_l)-amt_l2) >= first_amount:#값이 다르면 리스트에 추가를 해준다.
            dic['no'] += 1
            dic["item"].append({'no':dic['no'],'type':'M','enter_price':coin_price,'amt':round(amt_l-amt_l2, 3)})
            msg = "없는 롱포지션 추가\n현재 가격으로 추가합니다.\n실제 구매하신 가격하고 다를수 있습니다."
            viewlist(msg)
        ...
        if max_revenue < -(target_revenue_rate*3):
            # MA + 스토캐스틱 신호 체크
            # 숏포지션 : MA 하락 + 스토캐스틱 과매수에서 하락 전환
            if (ma5[0] < ma20 and ma5[2] < ma5[1] and ma5[1] > ma5[0] and 
                stoch_k > 80 and stoch_k < prev_stoch_k):
                data = binanceX.create_order(Target_Coin_Ticker, 'market', 'sell', amount_s, None, {'positionSide': 'SHORT'})
                buy_price = float(data['average']) #구매가격
                dic['no'] += 1
                dic["item"].append({'no':dic['no'],'type':'N','enter_price':buy_price,'amt':round(-amount_s, 3)})
                msg = "\n"+Target_Coin_Symbol+" 숏 포지션 구매22 (MA+Stoch)"
                viewlist(msg)
            
            # 롱포지션 : MA 상승 + 스토캐스틱 과매도에서 상승 전환
            if (ma5[0] > ma20 and ma5[2] > ma5[1] and ma5[1] < ma5[0] and 
                stoch_k < 20 and stoch_k > prev_stoch_k):
                data = binanceX.create_order(Target_Coin_Ticker, 'market', 'buy', amount_l, None, {'positionSide': 'LONG'})
                buy_price = float(data['average']) #구매가격
                dic['no'] += 1
                dic["item"].append({'no':dic['no'],'type':'N','enter_price':buy_price,'amt':round(amount_l, 3)})
                msg = "\n"+Target_Coin_Symbol+" 롱 포지션 구매22 (MA+Stoch)"
                viewlist(msg)
        
        # 마지막이 숏이고 롱을 사야 하는순간일경우 (MA+스토캐스틱)
        elif (max_revenue < -target_revenue_rate and dic["item"][max_revenue_index]["amt"] < 0 and 
              ma5[0] > ma20 and ma5[2] > ma5[1] and ma5[1] < ma5[0] and 
              stoch_k < 20 and stoch_k > prev_stoch_k):
            data = binanceX.create_order(Target_Coin_Ticker, 'market', 'buy', amount_l, None, {'positionSide': 'LONG'})
            buy_price = float(data['average']) #구매가격
            dic['no'] += 1
            dic["item"].append({'no':dic['no'],'type':'N','enter_price':buy_price,'amt':round(amount_l, 3)})
            msg = "\n"+Target_Coin_Symbol+" 롱 포지션 구매33 (MA+Stoch)"
            viewlist(msg)

        # 마지막이 롱이고 숏을 사야 하는 순간 (MA+스토캐스틱)
        elif (max_revenue < -target_revenue_rate and dic["item"][max_revenue_index]["amt"] > 0 and 
              ma5[0] < ma20 and ma5[2] < ma5[1] and ma5[1] > ma5[0] and 
              stoch_k > 80 and stoch_k < prev_stoch_k):
            data = binanceX.create_order(Target_Coin_Ticker, 'market', 'sell', amount_s, None, {'positionSide': 'SHORT'})
            ...
            dic['no'] += 1
            dic["item"].append({'no':dic['no'],'type':'N','enter_price':buy_price,'amt':round(-amount_s, 3)})
            msg = "\n"+Target_Coin_Symbol+" 숏 포지션 구매33 (MA+Stoch)"
            viewlist(msg)
            ...
            for i,item in enumerate(reversed(dic["item"])):
                if item["amt"] < 0: #숏포지션
                    # MA + 스토캐스틱 청산 신호: MA 상승 + 스토캐스틱 과매도에서 상승 전환
                    if (ma5[0] > ma20 and ma5[2] > ma5[1] and ma5[1] < ma5[0] and 
                        stoch_k < 20 and stoch_k > prev_stoch_k):
                    ...
                    if (ma5[0] < ma20 and ma5[2] < ma5[1] and ma5[1] > ma5[0] and 
                        ...
                        if revenue_rate2 >= target_revenue_rate:
                            ...
                            dic["today"] += my_rate_dollar
                            msg += "\n"+Target_Coin_Symbol+" 롱 포지션 판매 (MA+Stoch)"
                            msg += "\n"+item["type"]+":"+str(round(sell_price, 1))+" 수량:"+str(round(item["amt"], 3))+" 수익:"+str(round(my_rate_dollar, 2))+"$"
            ...
            if len(dic["item"]) > 0 and cap > 0.0:
                ...
                for i, item in enumerate(dic["item"]):
                    if item["amt"] > 0:
                        dic["item"][i]["price"] = round(((item["enter_price"]*abs(item["amt"]))-cap)/abs(item["amt"]), 2)
                    else:
                        dic["item"][i]["price"] = round(((item["enter_price"]*abs(item["amt"]))+cap)/abs(item["amt"]), 2)
            ...
            if isbuy != None:
                ...
                if isbuy == "short":
                    ...
                    if (ma5[0] < ma20 and ma5[2] < ma5[1] and ma5[1] > ma5[0] and 
                        ...
                        dic['no'] += 1
                        dic["item"].append({'no':dic['no'], 'type':'N','enter_price':buy_price,'amt':round(-amount_s, 3)})
                        msg += "\n"+Target_Coin_Symbol+" 숏 포지션 구매44 (MA+Stoch)"
                        viewlist(msg)
                
                elif isbuy == "long":
                    # MA + 스토캐스틱 신호 확인 후 롱 진입
                    if (ma5[0] > ma20 and ma5[2] > ma5[1] and ma5[1] < ma5[0] and 
                        stoch_k < 20 and stoch_k > prev_stoch_k):
                        data = binanceX.create_order(Target_Coin_Ticker, 'market', 'buy', amount_l, None, {'positionSide': 'LONG'})
                        buy_price = float(data['average']) #구매가격
                        dic['no'] += 1
                        dic["item"].append({'no':dic['no'], 'type':'N','enter_price':buy_price,'amt':round(amount_l, 3)})
                        msg += "\n"+Target_Coin_Symbol+" 롱 포지션 구매44 (MA+Stoch)"
                        viewlist(msg)
