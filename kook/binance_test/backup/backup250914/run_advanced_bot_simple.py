...
def run_sliding_window_test():
    ...
    # 모델 로더 초기화 (스크립트 위치 기준으로 models 폴더 사용)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    # 기간별 테스트 (15일 단위 슬라이딩 윈도우) - 1단계만 테스트
    test_periods = [
        {
            'name': '기간 1',
    ...
    for period in test_periods:
        ...
        try:
            ...
            if model_data is not None:
                ...
                # 로드된 모델의 파라미터를 사용 (학습된 파라미터)
                model_info = model_data.get('model_info')
                if model_info and 'params' in model_info:
                    loaded_params = model_info['params']
                    bot.params.update(loaded_params)
                    logger.info(f"{period['name']}: 학습된 파라미터 로드 - stop_loss: {bot.params['stop_loss_pct']}, take_profit: {bot.params['take_profit_pct']}")
                ...
                # 모델이 없으면 파라미터 튜닝 후 새로 훈련하고 지정된 이름으로 저장
                logger.warning(f"{period['name']}: 모델 없음, 파라미터 튜닝 후 새로 훈련...")
                
                # 1단계: 파라미터 튜닝
                logger.info(f"{period['name']}: 파라미터 튜닝 시작...")
                tune_result = bot.auto_tune_parameters(train_df, n_trials=50)  # 10번만 튜닝 (빠르게)
                logger.info(f"{period['name']}: 파라미터 튜닝 완료 - 최고 점수: {tune_result['best_score']:.4f}")
                # 최적 파라미터 상세 출력
                logger.info(f"{period['name']}: 최적 파라미터 상세:")
                ...
                # 2단계: 최적 파라미터로 모델 훈련
                logger.info(f"{period['name']}: 최적 파라미터로 모델 훈련 시작...")
                ...
                # 지정된 모델명으로 저장 (현재 디렉토리의 models 폴더)
                try:
...
def main():
    ...
    print("1단계만 테스트 (2024-01-01~01-15 학습, 2024-01-16~01-30 테스트)")
