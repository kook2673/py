...
class MAAdaptiveStrategySystem:
    ...
    def __init__(self, initial_capital: float = 100000):
        ...
        # 전략별 자본 (2개 전략: 롱 2개, 각 25%)
        # 전체 자본의 50%만 사용, 나머지 50%는 현금 보유
        investment_capital = self.initial_capital * 0.5  # 투자 자본 (50%)
        self.strategy_capitals = {
            'moving_average_long': investment_capital * 0.5,    # 25% (50%의 50%)
            'macd_long': investment_capital * 0.5              # 25% (50%의 50%)
        }
        
        # 전략별 거래 기록 (2개 전략: 롱 2개)
        self.strategy_trades = {
            'moving_average_long': [],
            'macd_long': []
        }
        
        # 전략별 성과 추적 (동적 자본 배분용) - 2개 전략: 롱 2개
    ...
    def backtest_strategy(self, data: pd.DataFrame, strategy_name: str) -> dict:
        ...
        # 초기 투자 자본은 그대로 유지 (재계산을 위해)
        initial_strategy_capital = self.strategy_capitals[strategy_name]
        
        capital = initial_strategy_capital
        position = 0
        entry_price = 0
        leveraged_value = 0
        trades = []
        ...
        for i in range(len(signals)):
            ...
            if position != 0:
                ...
                if self.check_stop_loss(position, entry_price, current_price, is_short):
                    ...
                    capital_change = new_capital - (leveraged_value / self.leverage)
                    capital = new_capital
                    position = 0
                    entry_price = 0
                    leveraged_value = 0
            ...
            if signal == 1 and position == 0:  # 매수 (롱) 또는 숏 매수
                ...
                capital = 0 # 포지션 진입 시 자본은 0으로 설정 (모두 투자)
        ...
        total_return = (capital - initial_strategy_capital) / initial_strategy_capital
    ...
    def run_continuous_backtest(self, start_year: int, end_year: int):
        ...
        print(f"📊 전략 수: 2개 (롱 2개, 각 25%)")
        print(f"💰 투자 자본: 50%, 현금 보유: 50%")
        ...
        # 전략별 누적 성과 추적 (연속성 유지) - 2개 전략: 롱 2개
        ...
        for year in range(start_year, end_year + 1):
            ...
            # 매년 균등 배분 (성과 기반 배분 비활성화) - 2개 전략: 롱 2개 (각 25%)
            # 전체 자본의 50%만 사용, 나머지 50%는 현금 보유
            investment_capital = current_capital * 0.5  # 투자 자본 (50%)
            self.strategy_capitals = {
                'moving_average_long': investment_capital * 0.5,    # 25% (50%의 50%)
                'macd_long': investment_capital * 0.5              # 25% (50%의 50%)
            }
            ...
            # 다음 해를 위한 자본 업데이트
            # 투자 자본 + 현금 자본 (50% 현금 보유)
            investment_final_capital = sum(result['final_capital'] for result in year_results.values())
            cash_capital = current_capital * 0.5  # 현금 자본 (50%)
            year_final_capital = investment_final_capital + cash_capital
            current_capital = year_final_capital
            
            # ⭐ 수정된 부분: 연도별 최종 자본을 결과에 저장
            year_results['total_final_capital'] = year_final_capital
            ...
            # 각 전략별 상세 성과 표시
            print(f"📊 {year}년 전략별 상세 성과:")
            print(f"   투자 자본: ${investment_capital:,.2f} (50%)")
            print(f"   현금 보유: ${cash_capital:,.2f} (50%)")
            print(f"   투자 수익: ${investment_final_capital:,.2f}")
            print(f"   투자 수익률: {((investment_final_capital - investment_capital) / investment_capital * 100):.2f}%")
            
            # 각 전략별 개별 성과
            for strategy_name, result in year_results.items():
                if strategy_name == 'total_final_capital': continue # 추가된 값은 건너뛰기
                initial_strategy_capital = investment_capital * 0.5  # $25,000
                final_strategy_capital = result['final_capital']
                strategy_return = ((final_strategy_capital - initial_strategy_capital) / initial_strategy_capital * 100)
                print(f"   {strategy_name:<20}: ${initial_strategy_capital:8,.0f} → ${final_strategy_capital:8,.0f} ({strategy_return:+6.1f}%)")
    ...
    def analyze_continuous_results(self, all_results: dict, initial_capital: float, final_capital: float):
        ...
        # 투자 vs 현금 분석
        final_investment = sum(result['final_capital'] for result in all_results[max(all_results.keys())].values() if isinstance(result, dict) and 'final_capital' in result)
        final_cash = initial_capital * 0.5  # 현금은 그대로 유지
        print(f"📊 최종 자본 구성:")
        print(f"   투자 자본: ${final_investment:,.2f} (투자 수익률: {((final_investment - initial_capital*0.5) / (initial_capital*0.5) * 100):.2f}%)")
        print(f"   현금 보유: ${final_cash:,.2f} (0%)")
        print(f"   총 자본: ${final_investment + final_cash:,.2f}")
        
        # 연도별 성과
        print(f"\n📅 연도별 성과:")
        print("-" * 60)
        
        # ⭐ 수정된 부분: 연도별 자본을 정확히 추적
        prev_capital = initial_capital
        
        for year in sorted(all_results.keys()):
            # 저장된 전체 최종 자본을 가져와서 계산
            year_final_capital = all_results[year].get('total_final_capital', 0)
            if year_final_capital == 0:
                year_return = 0
            else:
                year_return = (year_final_capital - prev_capital) / prev_capital
            
            print(f"{year}년: ${year_final_capital:,.2f} ({year_return*100:+.2f}%)")
            prev_capital = year_final_capital
...
def load_data(symbol: str, year: int) -> pd.DataFrame:
    """데이터 로드"""
    filename = f"data/{symbol}/1h/{symbol}_1h_{year}.csv"
