...
# 로그 파일 설정
log_file = os.path.join(logs_dir, "advanced_ma_ml_bot.log")
logging.basicConfig(
    level=logging.INFO, 
...
class AdvancedMAMLBot:
    ...
    def generate_signal(self, df: pd.DataFrame, features: List[str]) -> Dict:
        ...
        if prediction == 1 and (ma_signal >= 0 or (ma_short == 0 and ma_long == 0)) and confidence > 0.55:
...
if __name__ == "__main__":
    ...
    # 데이터 로드 (예시)
    try:
        # 데이터 파일 경로 설정 (사용자 환경에 맞게 수정)
        data_file = 'data/BTCUSDT/5m/BTCUSDT_5m_2024.csv'  # 기본 경로
