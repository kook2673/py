...
class MAAdaptiveStrategySystem:
    ...
    def __init__(self, initial_capital: float = 100000):
        ...
        # 전략별 자본 (4개 전략: 롱 2개, 숏 2개)
        # 초기에는 균등 배분, 이후 성과에 따라 동적 조정
        self.strategy_capitals = {
            'moving_average_long': self.initial_capital * 0.25,  # 이동평균 롱 (25%)
            'macd_long': self.initial_capital * 0.25,            # MACD 롱 (25%)
            'macd_short': self.initial_capital * 0.25,           # MACD 숏 (25%)
            'bb_short': self.initial_capital * 0.25              # 볼린저밴드 숏 (25%)
        }
        
        # 전략별 거래 기록 (4개 전략: 롱 2개, 숏 2개)
        ...
        # 전략별 성과 추적 (동적 자본 배분용) - 4개 전략: 롱 2개, 숏 2개
    ...
    def run_continuous_backtest(self, start_year: int, end_year: int):
        ...
        # 전략별 누적 성과 추적 (연속성 유지) - 4개 전략: 롱 2개, 숏 2개
        ...
        for year in range(start_year, end_year + 1):
            ...
            # 매년 균등 배분 (성과 기반 배분 비활성화) - 4개 전략: 롱 2개, 숏 2개
            self.strategy_capitals = {
                'moving_average_long': current_capital * 0.25,
                'macd_long': current_capital * 0.25,
                'macd_short': current_capital * 0.25,
                'bb_short': current_capital * 0.25
            }
