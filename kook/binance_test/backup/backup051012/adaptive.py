...
# UTF-8 인코딩으로 표준 출력 설정 (Windows PowerShell 오류 해결)
...
class MAAdaptiveStrategySystem:
    ...
    def __init__(self, initial_capital: float = 100000, enabled_strategies: list = None, config_file: str = None):
        ...
        if config_file and os.path.exists(config_file):
            ...
            # 기본값: 엘리트 조합 롱 전략과 최고의 숏 전략만 실행
            self.enabled_strategies = [
                'elite_long', 
                'bb_short'
            ]
            self.strategy_descriptions = {
                'elite_long': 'momentum_long과 bb_long 신호 동시 발생 시 진입',
                'bb_short': '볼린저밴드 숏 전략'
            }
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # MACD 롱 신호 (MA 비율 적용) & 랜덤 요소 제거
        df['macd_long_signal'] = 0
        macd_long_entry = macd_cross_up & (df['long_probability'] >= 0.7)
        df.loc[macd_long_entry, 'macd_long_signal'] = 1
        df.loc[macd_cross_down, 'macd_long_signal'] = -1
        
        # MACD 숏 신호 (MA 비율 적용) & 랜덤 요소 제거
        df['macd_short_signal'] = 0
        macd_short_entry = macd_cross_down & (df['short_probability'] >= 0.7)
        ...
        trend_long_entry = trend_long_condition & (df['long_probability'] >= 0.7)
        ...
        trend_short_entry = trend_short_condition & (df['short_probability'] >= 0.7)
        ...
        # 볼린저 밴드 롱 신호 (MA 비율 적용) & 랜덤 요소 제거
        df['bb_long_signal'] = 0
        bb_long_condition = df['close'] <= df['bb_lower']  # 하단 밴드 터치 시 롱 진입
        bb_long_entry = bb_long_condition & (df['long_probability'] >= 0.7)
        ...
        # 볼린저 밴드 숏 신호 (MA 비율 적용) & 랜덤 요소 제거
        df['bb_short_signal'] = 0
        bb_short_condition = df['close'] >= df['bb_upper']  # 상단 밴드 터치 시 숏 진입
        bb_short_entry = bb_short_condition & (df['short_probability'] >= 0.7)
        ...
        # 7. 신호 조합(Combination) 전략 추가
        df = self.add_combination_signals(df)

        # 8. 엘리트 조합 전략 신호 추가
        print("   - 엘리트(Elite) 조합 전략 신호 계산 중...")
        df['elite_long_signal'] = 0
        # 진입: momentum_long과 bb_long 신호가 동시에 1일 때
        elite_long_entry = (df['momentum_long_signal'] == 1) & (df['bb_long_signal'] == 1)
        df.loc[elite_long_entry, 'elite_long_signal'] = 1
        # 청산: 둘 중 하나라도 청산 신호를 보낼 때
        elite_long_exit = (df['momentum_long_signal'] == -1) | (df['bb_long_signal'] == -1)
        df.loc[elite_long_exit, 'elite_long_signal'] = -1
        
        return df
    
    def add_combination_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        """여러 전략의 신호가 일치할 때 진입하는 조합 전략 신호를 추가합니다."""
        print("   - 조합(Combination) 전략 신호 계산 중...")
        
        # 조합 전략 계산에 사용될 기본 롱/숏 전략 이름 목록
        base_long_strategies = [
            'momentum_long', 'scalping_long', 'macd_long',
            'moving_average_long', 'trend_long', 'bb_long'
        ]
        base_short_strategies = [
            'momentum_short', 'scalping_short', 'macd_short',
            'moving_average_short', 'trend_short', 'bb_short'
        ]

        # 해당 신호 컬럼들이 df에 있는지 확인
        long_signal_cols = [f"{s}_signal" for s in base_long_strategies if f"{s}_signal" in df.columns]
        short_signal_cols = [f"{s}_signal" for s in base_short_strategies if f"{s}_signal" in df.columns]

        # 각 행에서 매수 신호(1)의 개수를 합산 (벡터화 연산)
        df['long_signal_count'] = (df[long_signal_cols] == 1).sum(axis=1)
        df['short_signal_count'] = (df[short_signal_cols] == 1).sum(axis=1)

        # 조합 롱 전략 신호
        min_long_signals_for_entry = 3  # 진입 조건: 최소 3개
        min_long_signals_for_exit = 1   # 청산 조건: 신호가 1개 이하로 감소

        df['combination_long_signal'] = 0
        df.loc[df['long_signal_count'] >= min_long_signals_for_entry, 'combination_long_signal'] = 1
        df.loc[df['long_signal_count'] <= min_long_signals_for_exit, 'combination_long_signal'] = -1
        
        # 조합 숏 전략 신호
        min_short_signals_for_entry = 3 # 진입 조건: 최소 3개
        min_short_signals_for_exit = 1  # 청산 조건: 신호가 1개 이하로 감소

        df['combination_short_signal'] = 0
        df.loc[df['short_signal_count'] >= min_short_signals_for_entry, 'combination_short_signal'] = 1
        df.loc[df['short_signal_count'] <= min_short_signals_for_exit, 'combination_short_signal'] = -1
...
def load_data(symbol: str, year: int) -> pd.DataFrame:
    """데이터 로드"""
    # 스크립트 파일의 위치를 기준으로 절대 경로 생성
    base_dir = os.path.dirname(os.path.abspath(__file__))
    filename = os.path.join(base_dir, 'data', symbol, '5m', f'{symbol}_5m_{year}.csv')
    
    if not os.path.exists(filename):
        print(f"[ERROR] 파일을 찾을 수 없습니다: {filename}")
        return None
    
    print(f"[INFO] {symbol} {year}년 데이터 로드 중...")
    df = pd.read_csv(filename)
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    df.set_index('timestamp', inplace=True)
    df = df.sort_index()
    
    print(f"[INFO] 데이터 로드 성공: {len(df):,}개 캔들")
    print(f"[INFO] 기간: {df.index[0].strftime('%Y-%m-%d')} ~ {df.index[-1].strftime('%Y-%m-%d')}")
    print(f"[INFO] 가격 범위: ${df['close'].min():.2f} ~ ${df['close'].max():.2f}")
...
def main():
    ...
    parser.add_argument('--year', type=int, default=2024, help='백테스트 연도 (기본값: 2024)')
    ...
    if args.list_strategies:
        ...
        all_strategies = [
            ...
            'bb_long', 'bb_short',
            'combination_long', 'combination_short',
            'elite_long'
        ]
        descriptions = {
            ...
            'bb_long': '볼린저밴드 롱 전략',
            'bb_short': '볼린저밴드 숏 전략',
            'combination_long': '3개 이상 롱 전략 신호 일치 시 진입',
            'combination_short': '3개 이상 숏 전략 신호 일치 시 진입',
            'elite_long': 'momentum_long과 bb_long 신호 동시 발생 시 진입'
        }
        for strategy in all_strategies:
            print(f"  {strategy:<25}: {descriptions.get(strategy, '설명 없음')}")
        return
