...
# -*- coding: utf-8 -*-
"""
MA 적응형 롱/숏 전략 시스템 v3
- 6개 전략 x 롱/숏 = 12개 전략
- MA 10일~100일 기반으로 롱/숏 비율 동적 조정 (0:100 ~ 100:0)
- 자산 100등분, 각 전략에 1%씩 배분 (총 12% 사용)
- 5배 레버리지 고배율 운영
- 성과 기반 동적 자본 배분
"""
...
class MAAdaptiveStrategySystem:
    ...
    def __init__(self, initial_capital: float = 100000):
        ...
        # 전략별 자본 (12개 전략, 각 1% 배분)
        # 자산을 100등분하여 각 전략에 1%씩 배분 (총 12% 사용)
        self.strategy_capitals = {
            'momentum_long': self.initial_capital * 0.01,       # 모멘텀 롱 (1%)
            'momentum_short': self.initial_capital * 0.01,      # 모멘텀 숏 (1%)
            'scalping_long': self.initial_capital * 0.01,       # 스캘핑 롱 (1%)
            'scalping_short': self.initial_capital * 0.01,      # 스캘핑 숏 (1%)
            'macd_long': self.initial_capital * 0.01,           # MACD 롱 (1%)
            'macd_short': self.initial_capital * 0.01,          # MACD 숏 (1%)
            'moving_average_long': self.initial_capital * 0.01, # 이동평균 롱 (1%)
            'moving_average_short': self.initial_capital * 0.01,# 이동평균 숏 (1%)
            'trend_long': self.initial_capital * 0.01,          # 트렌드 롱 (1%)
            'trend_short': self.initial_capital * 0.01,         # 트렌드 숏 (1%)
            'bb_long': self.initial_capital * 0.01,             # 볼린저밴드 롱 (1%)
            'bb_short': self.initial_capital * 0.01             # 볼린저밴드 숏 (1%)
        }
        ...
        # 전략별 성과 추적 (동적 자본 배분용) - 12개 전략
        self.strategy_performance = {
            'momentum_long': {'trades': 0, 'wins': 0, 'total_return': 0},
            'momentum_short': {'trades': 0, 'wins': 0, 'total_return': 0},
            'scalping_long': {'trades': 0, 'wins': 0, 'total_return': 0},
            'scalping_short': {'trades': 0, 'wins': 0, 'total_return': 0},
            'macd_long': {'trades': 0, 'wins': 0, 'total_return': 0},
            'macd_short': {'trades': 0, 'wins': 0, 'total_return': 0},
            'moving_average_long': {'trades': 0, 'wins': 0, 'total_return': 0},
            'moving_average_short': {'trades': 0, 'wins': 0, 'total_return': 0},
            'trend_long': {'trades': 0, 'wins': 0, 'total_return': 0},
            'trend_short': {'trades': 0, 'wins': 0, 'total_return': 0},
            'bb_long': {'trades': 0, 'wins': 0, 'total_return': 0},
    ...
    def run_continuous_backtest(self, start_year: int, end_year: int):
        ...
        print(f"📊 전략 수: 12개 (각 1% 배분, 총 12% 사용)")
        print(f"⚡ 레버리지: {self.leverage}배 (고배율)")
        ...
        for year in range(start_year, end_year + 1):
            ...
            if year > start_year:
                ...
                # 첫 해는 균등 배분 (12개 전략, 각 1%)
                self.strategy_capitals = {
                    'momentum_long': current_capital * 0.01,
                    'momentum_short': current_capital * 0.01,
                    'scalping_long': current_capital * 0.01,
                    'scalping_short': current_capital * 0.01,
                    'macd_long': current_capital * 0.01,
                    'macd_short': current_capital * 0.01,
                    'moving_average_long': current_capital * 0.01,
                    'moving_average_short': current_capital * 0.01,
                    'trend_long': current_capital * 0.01,
                    'trend_short': current_capital * 0.01,
                    'bb_long': current_capital * 0.01,
                    'bb_short': current_capital * 0.01
                }
    ...
    def rebalance_capitals_from_performance(self, cumulative_performance: dict, current_capital: float):
        """누적 성과를 기반으로 자본 재배분 (1% 기반 하이브리드 접근법)"""
        # 최근 성과에 더 가중치를 주는 방식
        recent_performance = {}
        ...
        if total_recent_positive > 0:
            # 하이브리드 배분: 50% 성과 기반, 50% 균등 배분 (각 1%)
            for strategy_name in self.strategy_capitals.keys():
                if strategy_name in recent_performance:
                    ...
                    # 균등 배분 (50%) - 1%
                    equal_capital = current_capital * 0.005  # 0.5%
                    self.strategy_capitals[strategy_name] = performance_capital + equal_capital
                else:
                    # 성과가 없는 전략은 균등 배분만 (1%)
                    self.strategy_capitals[strategy_name] = current_capital * 0.01
        else:
            # 모든 전략이 손실이면 균등 배분 (각 1%)
            for strategy_name in self.strategy_capitals.keys():
                self.strategy_capitals[strategy_name] = current_capital * 0.01
