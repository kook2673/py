...
class Top4StrategySystem:
    ...
    def execute_trade(self, strategy_name: str, signal: int, price: float, timestamp: pd.Timestamp):
        ...
        if signal == 1 and current_position == 0:  # 매수
            ...
            # 레버리지 수익/손실 계산 (효율적인 검색)
            last_buy = next((t for t in reversed(self.strategy_trades[strategy_name]) if t['action'] == 'BUY'), None)
            if last_buy:
            ...
            self.strategy_positions[strategy_name] = 0  # 포지션 명시적 초기화
    ...
    def calculate_portfolio_value(self, current_price: float) -> float:
        ...
        for strategy_name in self.strategy_capitals:
            ...
            # 포지션 가치 계산
            position = self.strategy_positions[strategy_name]
            if position > 0:  # 롱 포지션
                # 마지막 BUY 거래 찾기
                last_buy_trade = next((t for t in reversed(self.strategy_trades[strategy_name]) if t['action'] == 'BUY'), None)
                if last_buy_trade:
                    entry_price = last_buy_trade['price']
                    pnl_shares = (current_price - entry_price) * position
                    position_value = (last_buy_trade['leveraged_value'] / self.leverage) + pnl_shares
                else:
                    ...
                    if i + 1 < len(trades):
                        ...
                        if buy_trade['action'] == 'BUY' and sell_trade['action'] == 'SELL':
                            ...
                            if pnl != 0:
                                ...
                                # fallback: 기존 방식
                                leveraged_pnl = (sell_trade['net_value'] - buy_trade['leveraged_value']) / buy_trade['leveraged_value']
                                actual_pnl = leveraged_pnl / self.leverage
                                total_pnl += actual_pnl
                                if actual_pnl > 0:
                                    winning_trades += 1
                
                win_rate = (winning_trades / (total_trades // 2)) * 100 if total_trades > 0 else 0
                avg_return = (total_pnl / self.base_capital_per_strategy) * 100 if total_trades > 0 else 0
