...
class MAAdaptiveStrategySystem:
    ...
    def __init__(self, initial_capital: float = 100000):
        ...
        # 전략별 자본 (6개 전략 x 롱/숏 = 12개 전략)
        # 초기에는 균등 배분, 이후 성과에 따라 동적 조정
        self.strategy_capitals = {
            'momentum_long': self.initial_capital * 0.0833,     # 모멘텀 롱
            'momentum_short': self.initial_capital * 0.0833,    # 모멘텀 숏
            'scalping_long': self.initial_capital * 0.0833,     # 스캘핑 롱
            'scalping_short': self.initial_capital * 0.0833,    # 스캘핑 숏
            'macd_long': self.initial_capital * 0.0833,         # MACD 롱
            'macd_short': self.initial_capital * 0.0833,        # MACD 숏
            'moving_average_long': self.initial_capital * 0.0833,  # 이동평균 롱
            'moving_average_short': self.initial_capital * 0.0833, # 이동평균 숏
            'trend_long': self.initial_capital * 0.0833,        # 트렌드 롱
            'trend_short': self.initial_capital * 0.0833,       # 트렌드 숏
            'bb_long': self.initial_capital * 0.0833,           # 볼린저밴드 롱
            'bb_short': self.initial_capital * 0.0833           # 볼린저밴드 숏
        }
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        scalping_long_buy = (df['volatility_5'] > 0.005) & (df['price_change_5'] > 0.003)
        ...
        scalping_short_buy = (df['volatility_5'] > 0.005) & (df['price_change_5'] < -0.003)
        ...
        df.loc[macd_cross_up, 'macd_long_signal'] = 1
        df.loc[macd_cross_down, 'macd_long_signal'] = -1
        
        # MACD 숏 신호
        df['macd_short_signal'] = 0
        df.loc[macd_cross_down, 'macd_short_signal'] = 1
        ...
        df.loc[df['ma_cross_up'], 'moving_average_long_signal'] = 1
        df.loc[df['ma_cross_down'], 'moving_average_long_signal'] = -1
        
        # 이동평균 숏 신호
        df['moving_average_short_signal'] = 0
        df.loc[df['ma_cross_down'], 'moving_average_short_signal'] = 1
        ...
        trend_long_entry = strong_uptrend & price_above_ma20 & positive_momentum
        ...
        # 볼린저 밴드 롱 신호
        df['bb_long_signal'] = 0
        bb_long_entry = df['close'] <= df['bb_lower']  # 하단 밴드 터치 시 롱 진입
        ...
        bb_short_entry = df['close'] >= df['bb_upper']  # 상단 밴드 터치 시 숏 진입
    ...
    def analyze_ma_ratios(self, data: pd.DataFrame, year: int):
        ...
        print(f"📊 {year}년 MA 기반 롱/숏 비율 분석:")
...
def main():
    ...
    parser.add_argument('--year', type=int, default=2022, help='백테스트 연도 (기본값: 2022)')
