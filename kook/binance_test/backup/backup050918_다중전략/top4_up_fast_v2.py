...
class Top4StrategySystemFast:
    ...
    def __init__(self, initial_capital: float = 100000):
        ...
        self.strategy_capitals = {
            'momentum': self.initial_capital * 0.15,        # 롱 모멘텀
            'scalping': self.initial_capital * 0.15,        # 롱 스캘핑
            'macd': self.initial_capital * 0.10,            # 롱 MACD
            'moving_average': self.initial_capital * 0.10,  # 롱 이동평균
            'short_scalping': self.initial_capital * 0.15,  # 숏 스캘핑
            'short_momentum': self.initial_capital * 0.10,  # 숏 모멘텀
            'trend_short': self.initial_capital * 0.15,     # 트렌드 숏
    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        short_scalping_sell = (df['volatility_5'] > 0.005) & (df['price_change_5'] < -0.003)
        short_scalping_buy = (df['volatility_5'] > 0.005) & (df['price_change_5'] > 0.003)
        df.loc[short_scalping_sell, 'short_scalping_signal'] = 1  # 숏 매수
        df.loc[short_scalping_buy, 'short_scalping_signal'] = -1  # 숏 매도
        
        # 6. 숏 모멘텀 전략 신호 (하락장 대응)
        df['short_momentum_signal'] = 0
        df.loc[df['momentum_20'] < -0.02, 'short_momentum_signal'] = 1  # 숏 매수
        df.loc[df['momentum_20'] > 0.02, 'short_momentum_signal'] = -1  # 숏 매도
        ...
        df.loc[trend_short_entry, 'trend_short_signal'] = 1  # 숏 매수
        df.loc[trend_short_exit, 'trend_short_signal'] = -1  # 숏 매도
        ...
        df.loc[bb_short_entry, 'bb_short_signal'] = 1  # 숏 매수
        df.loc[bb_short_exit, 'bb_short_signal'] = -1  # 숏 매도
    ...
    def backtest_strategy(self, data: pd.DataFrame, strategy_name: str) -> dict:
        ...
        if position > 0:
            ...
            buy_cost = leveraged_value
            pnl = net_value - buy_cost
            original_capital = buy_cost / self.leverage
            capital_change = pnl
            new_capital = max(0, original_capital + capital_change)
