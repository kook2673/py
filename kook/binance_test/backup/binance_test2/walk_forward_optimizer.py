...
def main(run_id=None):
    ...
    test_days = 7   # 3일치 데이터로 검증
    step_days = 7   # 3일씩 창문을 이동
    ...
    if all_oos_results:
        ...
        if all_equity_curves:
            ...
            combined_equity = combined_equity.groupby(combined_equity.index).mean()
            combined_equity.to_csv(EQUITY_CURVE_CSV, header=['equity'])
