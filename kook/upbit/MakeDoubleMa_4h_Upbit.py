...
#파일 경로입니다.
coinfindma_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "FindDoubleMaList_Upbit_4h.json") #내 PC에서 체크 할때 
#coinfindma_file_path = "/var/autobot/FindDoubleMaList_Upbit_4h.json" #서버에 업로드 할 때!
...
msg = "업비트 더블 이동평균 최적 이평선(4h) 계산 시작!!"
print(msg)
...
for ticker in Tickers:
    ...
    for coin_data in InvestCoinList:
        ...
        #일봉 정보를 가지고 온다!
        df = pyupbit.get_ohlcv(coin_ticker,interval="minute240",count=GetCount+2, period=0.3)
...
msg = "업비트 더블 이동평균 최적 이평선(4h) 계산 끝!!"
print(msg)
line_alert.SendMessage(msg)
